"""BED01 approved-source extraction and independent temporary-scene proof.
...
OLD = PRODUCT / "Baxter-Casablanca-BED01-derived-drawing.ifc"
APPROVAL = ROOT / "pipeline/decisions/bed01-drawing-approval.json"
OLD_MANIFEST = PRODUCT / "master-bedroom-bonsai-drawing-manifest.json"
SINGLE = OUT / "BED01-product.ifc"
RECIPE = OUT / "scene-recipe.json"
REPORT = OUT / "validation.json"
GUID = "3IQBEqO5vDI8Z9k1Ltge_N"
...
def prepare():
    ...
    assert pkg.sha256(OLD) == approval["derived_ifc_artifact"]["sha256"]
    assert not SINGLE.exists(), "Existing package requires explicit inspected retry"
    old_manifest = json.loads(OLD_MANIFEST.read_text())
    views = {v["view"]: {"annotation_guid": v["linework_annotation_global_id"],
    ...
    protected = [pkg.record(p) for p in [FORMAL, OLD, APPROVAL, OLD_MANIFEST,
                 PRODUCT / "official-dwg-review-reference.json", PRODUCT / "official-dwg-review-manifest.json",
    ...
    report = {"product": "BED01", "target_guid": GUID, "verdict": "pending_bonsai_scene",
              ...
              "views": [{"view": v["view"], "drawing_guid": v["drawing_global_id"],
                         "annotation_guid": v["linework_annotation_global_id"], "old_svg": v["svg"]}
...
def scene():
    ...
    import create_bed01_master_bedroom_drawings as drawing
    ...
    temporary = Path(tempfile.mkdtemp(prefix="bed01-pure-package-scene-"))
    ...
    write(REPORT, report)
    try:
        report["provider"] = {"version": list(bridge.bl_info["version"]), "port": 9885,
        ...
        for v in report["views"]:
            ...
            assert len(refs) == 1
            refs[0].Location = str(OUT / f"BED01-SCENE-{v['view'].upper()}.svg")
        ...
        for v in report["views"]:
            ...
            svg = OUT / f"BED01-SCENE-{v['view'].upper()}.svg"
            styles = drawing.style_svg(svg, v["annotation_guid"], v["view"])
