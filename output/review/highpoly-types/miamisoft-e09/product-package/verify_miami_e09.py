"""Disk-only verification of MIAMI-E09 package and regenerated approved blue lines."""
...
def main():
    ...
    for v in report["views"]:
        svg = task.OUT / f"MIAMI-E09-SCENE-{v['view'].upper()}.svg"
        old = line_geometry(task.ROOT / v["old_svg"]["path"], v["annotation_guid"])
        new = line_geometry(svg, v["annotation_guid"])
        if v["view"] == "side":
            old_hidden = [e for e in ET.parse(task.ROOT / v["old_svg"]["path"]).getroot().iter() if e.get("data-line-role") == "hidden"]
            new_hidden = [e for e in ET.parse(svg).getroot().iter() if e.get("data-line-role") == "hidden"]
            assert len(old_hidden) == len(new_hidden) == 259
            assert all("stroke-dasharray:2.4,1.5" in e.get("style", "") for e in new_hidden)
        ...
        single_svg = task.OUT / f"MIAMI-E09-SINGLE-{v['view'].upper()}.svg"
    ...
    task.write(task.OUT / "manifest.json", {
        "schema_version": 1, "profile_key": "miamisoft-e09", "display_name": "Baxter Miami Soft E09",
        "status": "validated_pending_visual_check", "target_global_id": task.GUID,
        "persistent_ifc": pkg.record(task.SINGLE), "source_kind": "native_dwg",
        "source_label_zh": "Baxter Miami Soft E09 官方原生二维 DWG 蓝线",
        "scope": "approved E09 dx/r native linework; project component offsets accepted; no force-fit",
        "source_dwg": pkg.record(task.ROOT / "output/review/highpoly-types/miamisoft-e09/official-source/official-download/Miami Soft_Divano_Abaco.dwg"),
        "official_download_url": source_data["official_product_cad"]["official_download_url"],
        ...
        "known_limitations": ["Project foot and bolster offsets are explicitly accepted; official linework is not force-fit."]})
    ...
    task.write(task.OUT / "handoff.json", {
        "product": "miami_e09", "status": "validated_pending_visual_check", "package": "manifest.json",
        "ifc": "MIAMI-E09-product.ifc", "scene_recipe": "scene-recipe.json", "validation": "validation.json",
        "new_files_only": True, "staged_baseline_modified": False, "legacy_deleted": False,
        "formal_modified": False, "bridge_port": 9886})
