"""Migrate approved folded HIMA01 black views into pure IFC, then verify pending R09 scene."""
...
OLD = PRODUCT / "Poliform-Hima-HIMA01-bonsai-isolated.ifc"
APPROVAL = ROOT / "pipeline/decisions/hima01-drawing-approval.json"
SINGLE = OUT / "HIMA01-product.ifc"
RECIPE = OUT / "scene-recipe.json"
REPORT = OUT / "validation.json"
GUID = "2xmcLzu1rDTeMzRuNxPDyE"
SOURCE_LABEL = "基于原始高模几何生成的简化图纸表达"
DWG_HASH = "f0e1b980aa102f3e06fe602f9e7a9b40db45d76e45200c08e4b8b4a0eec1d523"
...
def prepare():
    ...
    approved.require_approval(json.loads(APPROVAL.read_text()), PRODUCT/"manifest.json")
    ...
    assert pkg.sha256(PRODUCT/"official-source/official-download/Poliform-HIMA-screen.dwg")==DWG_HASH
    ...
    definitions={"plan":"FFL PLAN", "front":"EL-05-14-R09-PY", "side":"EL-05-17-R09-NX"}
    ...
    for view,name in definitions.items():
        ...
        drawing.GlobalId=ifcopenshell.guid.new();drawing.Name=f"HIMA01-SCENE-{view.upper()}"
        ...
        if view=="plan":
            drawing.ObjectPlacement.RelativePlacement.Location.Coordinates=(-5600.0,-2050.0,1700.0)
            block.XLength=4000.0;block.YLength=4200.0;block.ZLength=2000.0
        elif view=="front":
            # Look along +Y from ahead of the whole folded screen, with world Z up.
            drawing.ObjectPlacement.RelativePlacement.Location.Coordinates=(-5600.0,-3050.0,1250.0)
            block.XLength=4000.0;block.YLength=2700.0;block.ZLength=2200.0
        else:
            drawing.ObjectPlacement.RelativePlacement.Location.Coordinates=(-5250.0,-2050.0,1250.0)
            block.XLength=4200.0;block.YLength=2700.0;block.ZLength=2200.0
        ...
        style=source.create_entity("IfcCurveStyle",Name="Hima 01 approved folded black",CurveWidth=source.create_entity("IfcPositiveLengthMeasure",0.35),CurveColour=color,ModelOrDraughting=True)
        source.create_entity("IfcStyledItem",Item=rep.Items[0],Styles=[style])
        annotation=source.create_entity("IfcAnnotation",GlobalId=ifcopenshell.guid.new(),Name=f"Hima 01 approved {view}",ObjectType="LINEWORK",ObjectPlacement=target.ObjectPlacement,Representation=source.create_entity("IfcProductDefinitionShape",Representations=[rep]))
        pset(source,annotation,"EPset_Annotation",{"Classes":"review-target-hima01 geometry-derived-simplified-proxy","TargetGlobalId":GUID,"SourceKind":"geometry_derived_simplified_proxy","ReferenceOnlyDwgSha256":DWG_HASH})
    ...
    pset(source,target,"Pset_Hima01ApprovedSource",{"SourceKind":"geometry_derived_simplified_proxy","SourceLabelZh":SOURCE_LABEL,"OfficialDwgSha256":DWG_HASH,"OfficialDownloadUrl":approved.OFFICIAL_DWG_URL,"OfficialDwgUse":"family and unfolded-state reference only","OfficialCadUsed":False,"SingleProductApprovalStatus":"approved","SceneApprovalStatus":"pending","FormalIfcWriteAllowed":False})
    pure,recipe,audit=build_pure_package(source,GUID,specs)
    recipe['annotation_import_adapter']={'script':'migrate_hima01.py','function':'exact_annotation_mesh','run_before':'each bpy.ops.bim.create_drawing after activate_drawing','geometry_source':'persisted pure IFC ApprovedPlan/ApprovedFront/ApprovedSide polylines only','strategy':'create distinct Blender mesh vertices for every stored polyline point and one edge for every successive pair; no merging or simplification','scope':'temporary Blender annotation meshes only; IFC representations and placement unchanged','installed_api':'Blender 4.5.3 LTS; Bonsai and IfcOpenShell 0.8.4; SvgWriter.draw_misc_annotation and draw_edge_annotation emit LINEWORK mesh edges','historical_cache_used':False}
    ...
    write(REPORT,{"product":"HIMA01","target_guid":GUID,"verdict":"pending_bonsai_scene","formal_sha256_before":FORMAL_HASH,
        ...
        "scene_camera_policy":"Pending R09 context cameras; Plan 1700 mm above 1008 mm product and below ceiling; Front from -Y, Side from +X. World Z remains up.",
...
def style_svg(svg, annotation_guid, view):
    ...
    assert count>0,"No HIMA01 generated annotation geometry"
...
def exact_annotation_mesh(model, pure, spec, view):
    ...
    mesh=bpy.data.meshes.new('HIMA01 exact persisted '+view)
...
def scene():
    ...
    temporary=Path(tempfile.mkdtemp(prefix="hima01-pure-package-scene-"));temp_ifc=temporary/"scene.ifc"
    ...
    write(REPORT,report)
    try:
        report["provider"]={"status":"supported","version":list(bridge.bl_info["version"]),"port":9894,"pid":__import__('os').getpid(),"blender":bpy.app.version_string,"ifcopenshell":ifcopenshell.version,"bridge_source":pkg.record(bridge.__file__)}
        ...
        for v in report["views"]:
            ...
            svg=OUT/f"HIMA01-SCENE-{v['view'].upper()}.svg"
