...
def main():
    ...
    for view,spec in json.loads(task.RECIPE.read_text())['views'].items():
        ...
        approved=candidate['views'][view]['review_simplified_official_outline_paths_mm']
        ...
        scale=50.0
        svg=task.OUT/f'GESSI54145-SCENE-{view.upper()}.svg';lines=svg_lines(svg,spec['annotation_guid']);assert lines
        ...
        single_svg=task.OUT/f'GESSI54145-SINGLE-{view.upper()}.svg';ET.ElementTree(thumb).write(single_svg,encoding='utf-8',xml_declaration=True)
    ...
    task.write(task.OUT/'manifest.json',{'schema_version':1,'profile_key':'gessi316-54145','display_name':'Gessi316 54145','status':'validated_pending_visual_check','target_global_id':task.GUID,'persistent_ifc':pkg.record(task.SINGLE),'scene_recipe':pkg.record(task.RECIPE),'source_kind':'native_dwg_review_simplification','source_label_zh':task.SOURCE_LABEL,'source_dwg':pkg.record(task.PRODUCT/'official-source/GPF5414500000G000_3.dwg'),'official_download_url':'https://gessistorage.blob.core.windows.net/zwa/GPF5414500000G000_arc.zip','approval_record':pkg.record(task.APPROVAL),'migration_authorization':pkg.record(task.ROOT/'output/review/approved-product-library/migration-authorization.json'),'single_product_approval_status':'approved','scene_approval_status':'pending','approval_label_zh':'单品已通过、场景待验收','formal_write_allowed':False,'formal_sha256':task.FORMAL_HASH,'geometry':{'physical_products':1,'body_representation_count':2,'original_body_preserved':True,'discarded_box_representation_count':1,'approved_views':['plan','front','side'],'annotations':0,'groups':0,'drawing_cameras':0,'spatial_geometry':0,'units_to_m':.001,'placement_matrix_project_units':state['placement']},'scene_outputs':checks,'single_product_views':{x['view']:x['single_product_svg'] for x in checks},'library_previews':{x['view']:x['single_product_thumbnail'] for x in checks},'external_style_dependencies':r['necessary_external_style_dependencies'],'validation':'validation.json','known_limitations':['Exact 54145 G000 DWG-based simplified outlines; not untouched DWG.','New real Bonsai scene SVGs are technical migration checks awaiting human scene approval.'],'preview_provenance':{'2d':'Exact persisted Approved views, local positive axes match user-approved single-product orientation; display-only framing and stroke styling.'}})
    task.write(task.OUT/'cleanup-proposal.json',{'status':'proposal_only_no_deletion','requires_user_confirmation':True,'legacy_files':[pkg.record(task.OLD),pkg.record(task.PRODUCT/'Gessi316-54145-bonsai-review.blend')],'temporary_directories':sorted(set([r['temporary_project_directory'],*r.get('previous_temporary_attempts',[])])),'keep':[str(task.SINGLE),str(task.RECIPE),'official-source/**','source and approval records','scene SVG/PNG and validation evidence'],'cleanup_performed':False})
    task.write(task.OUT/'handoff.json',{'product':'gessi54145','status':'validated_pending_visual_check','package':'manifest.json','ifc':'GESSI54145-product.ifc','scene_recipe':'scene-recipe.json','validation':'validation.json','scene_approval_status':'pending','single_product_approval_status':'approved','new_files_only':True,'staged_baseline_modified':False,'legacy_deleted':False,'formal_modified':False,'bridge_port':9892})
