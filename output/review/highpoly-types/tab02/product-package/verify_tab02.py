...
import json,copy,subprocess,re,xml.etree.ElementTree as ET
...
def main():
    ...
    baseline_cache=json.loads((task.OUT/'formal-schema-baseline.json').read_text());assert baseline_cache['formal_sha256']==pkg.sha256(task.FORMAL)
    def stable_errors(logger):
        return Counter((x.get('attribute',''),re.sub(r'#\d+','#STEP',re.sub(r'\s+',' ',x['message'])),pkg.fingerprint(x['instance'])) for x in logger.statements)
    baseline_errors=Counter({tuple(x['key']):x['count'] for x in baseline_cache['errors']});scene_errors=stable_errors(scene_logger)
    ...
    r['baseline_inherited_errors']={'formal_count':sum(baseline_errors.values()),'temporary_scene_count':len(scene_logger.statements),'new_errors':0,'comparison':'Counter of attribute, whitespace/STEP-normalized error message, and stable instance content fingerprint','exact_baseline_match':True,'baseline_evidence':pkg.record(task.OUT/'formal-schema-baseline.json')}
    ...
    assert candidate['semantic_segmentation_sha256']==pkg.sha256(task.PRODUCT/'tab02-semantic-segmentation.json')
    ...
    r['annotation_import_adapter']={**recipe['annotation_import_adapter'],'script_evidence':pkg.record(task.OUT/'migrate_tab02.py'),'saved_source_representation_unchanged':True}
    ...
    for view,spec in json.loads(task.RECIPE.read_text())['views'].items():
        ...
        semantic=json.loads((task.PRODUCT/'tab02-semantic-segmentation.json').read_text())['views'][view]
        assert approved==[x['path_mm'] for x in semantic]
        assert candidate['views'][view]['semantic_paths']==semantic
        if view!='plan':assert all(len(x['path_mm'])==2 for x in semantic if x['component']=='central_column')
        ...
        scale=50.0
        svg=task.OUT/f'TAB02-SCENE-{view.upper()}.svg';lines=svg_lines(svg,spec['annotation_guid']);assert lines
        ...
        for e in lines:
            display=copy.deepcopy(e);display.set('style','stroke:#111820;stroke-width:0.045;fill:none;stroke-linecap:round;stroke-linejoin:round');thumb.append(display)
        points=np.array([p for path in approved for p in path]);lo=points.min(axis=0);hi=points.max(axis=0)
        single_svg=task.OUT/f'TAB02-SINGLE-{view.upper()}.svg';ET.ElementTree(thumb).write(single_svg,encoding='utf-8',xml_declaration=True)
    ...
    r['independent_validation']={'schema_errors':0,'temporary_scene_schema_errors':len(scene_logger.statements),'new_temporary_scene_schema_errors':0,'pure_state':state,'view_checks':checks,'all_formal_body_and_placement_count':714,'protected_files_unchanged':True,'recipe_runtime_geometry_source':'pure IFC only','single_product_envelope_and_origin_preserved':True}
    r['verdict']='validated_pending_visual_check';task.write(task.REPORT,r)
    task.write(task.OUT/'manifest.json',{'schema_version':1,'profile_key':'tab02','display_name':'RODA Bernardo 367 TAB02','status':'validated_pending_visual_check','target_global_id':task.GUID,'persistent_ifc':pkg.record(task.SINGLE),'scene_recipe':pkg.record(task.RECIPE),'source_kind':'geometry_derived_simplified_proxy','source_label_zh':task.SOURCE_LABEL,'source_reference':pkg.record(task.PRODUCT/'official-source/source-access-record.json'),'official_cad_used':False,'approval_record':pkg.record(task.APPROVAL),'migration_authorization':pkg.record(task.ROOT/'output/review/approved-product-library/migration-authorization.json'),'single_product_approval_status':'approved','scene_approval_status':'pending','approval_label_zh':'单品已通过、场景待验收','formal_write_allowed':False,'formal_sha256':task.FORMAL_HASH,'geometry':{'physical_products':1,'body_representation_count':1,'original_body_preserved':True,'discarded_box_representation_count':0,'approved_views':['plan','front','side'],'annotations':0,'groups':0,'drawing_cameras':0,'spatial_geometry':0,'units_to_m':.001,'placement_matrix_project_units':state['placement']},'scene_outputs':checks,'single_product_views':{x['view']:x['single_product_svg'] for x in checks},'library_previews':{x['view']:x['single_product_thumbnail'] for x in checks},'external_style_dependencies':r['necessary_external_style_dependencies'],'validation':'validation.json','known_limitations':['Approved semantic component black outlines from original Body; official CAD not acquired, catalogue used only for identity and dimensions.','New real Bonsai scene SVGs are technical migration checks awaiting human scene approval.'],'preview_provenance':{'2d':'Product GlobalId annotation LINEWORK cropped from verified real Bonsai Scene SVG; display-only framing; elevations preserve world +Z up.'}})
    task.write(task.OUT/'cleanup-proposal.json',{'status':'proposal_only_no_deletion','requires_user_confirmation':True,'legacy_files':[pkg.record(task.OLD),pkg.record(task.PRODUCT/'RODA-Bernardo-367-TAB02-bonsai-review.blend')],'temporary_directories':sorted(set([r['temporary_project_directory'],*r.get('previous_temporary_attempts',[])])),'keep':[str(task.SINGLE),str(task.RECIPE),'official-source/**','source and approval records','scene SVG/PNG and validation evidence'],'cleanup_performed':False})
    task.write(task.OUT/'handoff.json',{'product':'tab02','status':'validated_pending_visual_check','package':'manifest.json','ifc':'TAB02-product.ifc','scene_recipe':'scene-recipe.json','validation':'validation.json','scene_approval_status':'pending','single_product_approval_status':'approved','new_files_only':True,'staged_baseline_modified':False,'legacy_deleted':False,'formal_modified':False,'bridge_port':9897})
