...
- `pipeline/decisions/elec-design-rules.csv`：集中记录已确认的电气规则：只用实体有线开关；主卧/次卧各设 AP，客厅/书房各用路由器且不设 AP并保留有线回程；入户门与主卧门口设置客厅、书房、餐厅三组双控；入户照明总控排除冰箱、网络、安防和其他持续供电回路；普通插座、电视点和开关统一以面板底边距完成地面 300/600/1300 mm。规则编译进 E-302/E-304 候选，但不自动写 IFC。
- `pipeline:elec-control-network`：把入户门 M08、主卧门 M07 和 R09/R14/R20/R22 Space 的已确认身份编译为 2 个门口控制协调区、2 个卧室 AP 房间区和 2 个客厅/书房路由器房间区，生成单页 500×400 mm SVG/PDF 与 JSON 门报告。门中心和 Space 包围盒中心只用于标明协调范围，显式标记为非最终 XY/Z 安装点，不写 IFC。
