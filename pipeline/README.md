...
- `pipeline:a106-ceiling-device-candidate`：把主卧、次卧烟感与吸顶 AP 及客厅烟感编译为 5 个平面坐标采用 50 mm 模度的精确候选；Z 轴直接服从既有天花下表面，不强行取 50 mm。逐点核验房间边界、梁、79 个既有灯具包络、RCP1 其他高位对象实际网格和既有天花下表面；烟感为同时覆盖“500 mm 无障碍”采用比单独距灯 200 mm 更保守的 500 mm 灯具净距。审核图强制移除旧 `Wall Plan-underlay.png`，只保留矢量墙线、Grid、灯具与候选标记，避免旧栅格纹理误导人审。5/5 已知几何门通过。正式 IFC 仍无完整装修后风口，故烟感距送风口 1500 mm/多孔送风 500 mm 保持 `unknown`，不写 IFC、不进入施工发布。
