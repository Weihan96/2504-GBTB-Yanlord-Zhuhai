...
- `pipeline:a102-demolition-candidate`：只读校验交付基础 DWG、拆除情况 PDF 和正式 IFC 的 SHA-256。脚本从 PDF 图面提取 11 个黄色轮廓“已经拆除”矩形和 2 个红色轮廓“计划拆除”矩形，排除图例；以 14 个 Grid 锚点拟合 PDF/项目坐标，最大残差不得超过 `0.5 mm`，再生成 13 个位于 `50 mm` 控制网格上的待审墙段。每条记录保存原始矩形、原始世界坐标、候选边界、依据、置信度和 `review_required=yes`，但 `formal_ifc_write_allowed=no`。Blender 辅助脚本只建立有真实深度的实体边框：D01–D11 红色、D12–D13 紫红色；它不使用 Wireframe、不启用 `show_in_front`、不修改或保存 IFC。D13 显式保护已调整门洞墙 `0hKdvAZkn1TejLgJhK_vDp` 及 Opening `1YxMx6s0r3ZPPohkRKXWbl`。
