...
- `pipeline:wfin-semantics-candidate`：只在 `build/candidates/` 生成 WFIN 设计意图候选，为 51 个既有 `CLADDING` 添加项目专用 `Pset_WallFinishIntent`；该 Pset 不是 buildingSMART 标准 Pset，不会替换既有材料关系。机械门固定核对 24 个 Tadelakt、27 个大白墙、世界几何 `0.0 mm`、材料关系不变和正式 IFC 哈希不变；未决系统参数、主卫干区缺失饰面几何与 3 个 C003 安装锚点写入 `wfin-open-issues.csv`。
...
- `pipeline:wfin-semantics-candidate`：在 `build/candidates` 中为 51 个既有 `CLADDING` 生成项目专用 `Pset_WallFinishIntent` 候选，记录已确认的 FinishCode、Space Reference/LongName 与仍待确认的产品系统、颜色、厚度、基层和防水节点。该 Pset 是明确说明的项目设计意图载体，不冒充 buildingSMART 标准 Pset；候选不替换现有 WW20/WW20BR 材料层，不移动几何，并以 `FormalIfcWriteAllowed=false` 阻止未决参数进入正式 IFC。四类开放项写入 `pipeline/decisions/wfin-open-issues.csv`。
