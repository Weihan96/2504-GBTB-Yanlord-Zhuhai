...
- `pipeline/scripts/rcp1_hvac_constraint_preview.py`：在当前 Bonsai 会话安装 `RCP1 HVAC` 侧栏、7 个 IFC 锚点和 2 条 Geometry Nodes 路线预览。移动设备/弯点只自动更新 Blender 曲线；“导出路线候选”只把当前有序锚点和世界坐标写到 `build/rcp1/hvac-route-preview-candidate.json`，不写 CSV、`.blend` 或 IFC。旧 HVAC 审查层自动隐藏，视图保持 Solid、真深度、X-Ray/Wireframe/`show_in_front` 关闭。
- `pipeline:rcp1-hvac-preview-audit`：用可传 `--tolerance-mm` 机械比较 Blender 导出的固定锚点、顺序和当前 IFC/路线约束图；临时弯点可插入但必须进入人审。当前无弯点候选为 2 条路线、5 个固定锚点、最大偏差 `0.000121 mm`，通过 `0.1 mm` 门；施工几何与正式 IFC 写入门继续关闭。
