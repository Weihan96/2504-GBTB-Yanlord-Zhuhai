...
- `pipeline:rcp1-hvac-remodel-candidate`：把 5 个正式 AC、旧 `.blend` 中 1 个无 GlobalId 的东侧机位、7 个开发商既有空调洞和 5 个旧方案管道对象纳入同一只读候选。脚本机械识别 12 个独立管道几何分支，并利用现有世界网格协调报告逐台排序洞口与管道邻近关系；邻近或相交均不解释为端口连接。输出只进入 `build/rcp1/`，在设备数量、服务房间、送回风、厂家接管位置和冷凝水排放点确认前禁止写入正式 IFC。
