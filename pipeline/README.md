...
- `pipeline:rcp1-legacy-base-audit`：通过 Blender 后台只读打开 SHA-256 `a8e0a94ef766667ad3749b96c158afdd538a330009b135e49ed461d1634c78ed` 的 `2504_lowpoly.blend`，对照当前正式 IFC。五条管线的独立分支数全部一致，组件包围尺寸最大差 `0.015289 mm`；当前十对管线/空调相交在旧底稿中 `10/10` 同样相交。旧文件同时存在厨房机位与 `1QBd...` 东侧额外空调对象；东侧 Blender 候选中心与三台正式 AC 求得的旧/新平移基准最大轴差 `4.938 mm`，但它没有正式 GlobalId，只能作为后续设备数量与位置设计的底稿。脚本不保存 blend，不写 IFC。
- `pipeline:rcp1-hvac-remodel-candidate`：把 5 个正式 AC、旧 `.blend` 中 1 个无 GlobalId 的东侧机位、7 个开发商既有空调洞和 5 个旧方案管道对象纳入同一只读候选。脚本机械识别 12 个独立管道几何分支，并利用现有世界网格协调报告逐台排序洞口与管道邻近关系；邻近或相交均不解释为端口连接。两个已确认风口同时作为局部 `-Y` 出风侧依据，Space 取样只生成直接服务房间候选并明确暴露客厅 R20 尚无直接设备或风口；跨房取样不得冒充已连接风管。输出只进入 `build/rcp1/`，在设备数量、服务房间、送回风、厂家接管位置和冷凝水排放点确认前禁止写入正式 IFC。
  - 客厅选项采用最短平面路径与 `10 mm` 墙体包络审计：A05 方案约 `2309.365 mm`、方向转角 `0°`，仅接触已拆 D11；A06 方案约 `1399.338 mm`，但需转 `135.922°`、穿现状墙且新增正式设备。该比较只推荐人审顺序，不创建风管、风口或设备。
