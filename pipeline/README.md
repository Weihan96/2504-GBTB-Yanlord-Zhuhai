...
- `pipeline:rcp1-route-blender-review`：在 `Z=3.12 m` 的独立审查层绘制 6 条青色“设备—洞口配对证据线”，红点标出未分配 H01，橙线标出 H06→H01 外部穿墙链待确认。图例明确这些线不是风管或冷媒管；所有对象保持 Solid、真深度、X-Ray 关闭、Wireframe 关闭和 `show_in_front=0`。脚本只修改 Blender 审查状态，不写 IFC。
