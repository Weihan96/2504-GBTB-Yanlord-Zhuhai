...
- `pipeline:a104-candidate`：只读生成 A-104 SVG/PDF 和 `pipeline/decisions/a104-door-window-review.csv`。候选编号按平面由北到南、同排由西到东确定，未人审前不写 IFC `Tag`。机械门验证 8 门、11 窗、5 个有宿主门、11 个有宿主窗、3 樘无宿主门，逐樘比较洞口宽高与 IFC 名义宽高，门限可传入且默认为 `0.1 mm`；同时检查编号重复、同宿主洞口重叠和生成标注碰撞。房间归属只作为基于当前 Space 几何的候选证据，不自动写回。Blender 审核由 `pipeline/scripts/a104_blender_review.py` 恢复完整几何上下文：普通门青色、窗蓝色、待审对象红色、家具黄色，Solid、X-Ray 关闭、`show_in_front=0`。
