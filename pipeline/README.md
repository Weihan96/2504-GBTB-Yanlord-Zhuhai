...
- `pipeline/decisions/elec-design-rules.csv`：集中记录已确认的电气规则：只用实体有线开关；主卧/次卧各设 AP，客厅与书房作为开放空间共用 1 个路由器且不设 AP并保留有线回程；入户门与主卧门口设置客厅、书房、餐厅三组双控；入户照明总控排除冰箱、网络、安防和其他持续供电回路；普通插座、电视点和开关统一以面板底边距完成地面 300/600/1300 mm。所有天花设备必须与灯位共同排版，优先服从空间主轴并采用共轴、等距、成组；偏位须登记冲突依据并经人审。卧室 AP 允许采用可下拆的磁吸齐平预埋底座，但辐射面不得被石膏板或金属遮挡。主卧、次卧、客厅烟感需求与小米烟雾报警器齐平预埋候选已登记，候选开孔/沉孔/预埋深度为 105/140/30 mm；厨房燃气报警器为必须配置，小米燃气报警器候选尺寸为 94/120/43 mm，但燃气公司确认型号前不得锁定专用开孔。两类探测器均只隐藏底座，进烟/进气口、指示灯和可拆卸主体必须外露。规则编译进只读协调候选，但不自动写 IFC。
...
- `pipeline:a106-ceiling-device-candidate`：把主卧、次卧烟感与吸顶 AP 及客厅烟感编译为 5 个精确候选。卧室候选采用既有灯位 X/Y 轴线的空交点；客厅现有交点均无法满足烟感 500 mm 净距，故采用既有灯网重复节距的延长交点。XY 保持整数毫米，Z 轴直接服从既有天花下表面。逐点核验房间边界、梁、79 个既有灯具包络、RCP1 其他高位对象实际网格和既有天花下表面；烟感为同时覆盖“500 mm 无障碍”采用比单独距灯 200 mm 更保守的 500 mm 灯具净距。审核图强制移除旧 `Wall Plan-underlay.png`，并按 A-102 登记排除全部 13 面 `DEMOLISH` 墙，只保留建成墙矢量、Grid、灯具与候选标记。5/5 已知几何门通过。正式 IFC 仍无完整装修后风口，故烟感距送风口 1500 mm/多孔送风 500 mm 保持 `unknown`，不写 IFC、不进入施工发布。
