...
- `pipeline:a105-candidate`：只读生成 A-105 SVG/PDF、21 个地坪候选登记和 8 樘门的门槛交接登记。18 块 `TerrazzoMosaicTile` 顶面均从世界三角面机械拟合，记录坡度、下坡方向、完成面最高/最低标高和拟合残差；3 个零厚度 FFL 参考面明确标为材料与构造厚度待定。门槛只关联门中心处实际覆盖的候选地坪，不猜材料、高差、防水或无宿主门身份。另把 `3ARl_CqPrCWQrA6_$W073W` Aircrete 板移交为独立施工语义审核项。命令不写正式 IFC。
- `pipeline/scripts/p0_blender_review.py`：把 A-104 与 A-105 待审项一次性恢复为 Blender 真深度场景。红色仅表示 A-104 门窗待审；蓝色为已识别湿区地砖；橙色边界表示材料/构造待定的零厚度 FFL 参考面；紫色为待定 Aircrete 板；家具保持黄色。普通墙、门、窗、洁具、排水和 Grid 保留作上下文，所有对象 `show_in_front=0`，Solid、X-Ray 关闭且不预选对象，避免橙色选择轮廓覆盖审核色。3 个位于 `Z=0` 的零厚度参考面以真深度橙色 Solid 边界代理显示，源平面临时隐藏，避免遮住下方真实湿区地砖；不使用 Wireframe。
