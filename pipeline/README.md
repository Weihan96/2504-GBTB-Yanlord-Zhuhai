...
- `pipeline:wfin-semantics-candidate`：只在 `build/candidates/` 生成 WFIN 设计意图候选，为 51 个既有 `CLADDING` 添加项目专用 `Pset_WallFinishIntent`；该 Pset 不是 buildingSMART 标准 Pset，不会替换既有材料关系。机械门核对 54 个连续饰面分段＝26 个 Tadelakt＋28 个大白墙、3 个跨材料边界对象保持 `MULTI_FINISH_SPLIT_REQUIRED`、世界几何 `0.0 mm`、材料关系不变和正式 IFC 哈希不变；未决分界审核、系统参数与 3 个 C003 安装锚点写入 `wfin-open-issues.csv`。
...
- `pipeline:wfin-candidate`：只读盘点 51 个有效 `CLADDING`，沿薄型垂直饰面的长轴与 Grid Space 边界机械求交，而不是仅用对象中心归属。现有对象展开为 54 个连续分段：26 个 Tadelakt、28 个大白墙；`0e0XOb$L18ZBVYJiQJrQ1p`、`3bMoS7bIT8wBmjqRvdPunu`、`06wFwLoDD6ie5iCTnc_yad` 跨越两种材料范围，保持为 3 个待审核拆分对象。该分段只覆盖既有 `CLADDING`，不冒充完整房间饰面：主卫干区另有返口、低墙顶面、门垛和跨区踢脚待真深度审核。命令生成对象表、分段表、500×400 mm SVG/PDF、项目专用意图 Pset 候选和 QA，不写正式 IFC。
