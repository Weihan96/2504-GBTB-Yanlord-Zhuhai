...
- `pipeline:space-reference-candidate`：只读生成 22 个 Space 的 `R01-R22` 候选登记。`R01` 固定为玄关，其余按 Space 世界包围盒中心相对全体中心的顺时针极角排序；机械门验证编号完整唯一、LongName 保留，并同时接受“写入前全部为空”或“写入后 22 个值与候选完全一致”两种合法状态，拒绝部分写入或编号漂移。命令不写正式 IFC。
- `pipeline:space-reference-semantics-candidate`：从确认后的登记生成 S003 原子 IFC 候选，为 22 个 Space 新增标准 `Pset_SpaceCommon.Reference`。Pset 与关系使用按 Space GlobalId 派生的确定性 GlobalId；候选门验证 `R01-R22` 完整唯一、玄关为 `R01`、LongName 与 Space 世界几何不变，以及新增 Root 边界精确为 22 个 Pset＋22 个关系。`pipeline:space-reference-postwrite` 在 Bonsai 保存/reload 后按语义、Root 集合与 `0.1 mm` 世界几何门核对正式 IFC。
- `pipeline:wfin-semantics-candidate`：只在 `build/candidates/` 生成 WFIN 设计意图候选，为 51 个既有 `CLADDING` 添加项目专用 `Pset_WallFinishIntent`；该 Pset 不是 buildingSMART 标准 Pset，不会替换既有材料关系。机械门核对 52 个当前意图分段＝24 个 Tadelakt＋28 个大白墙；用户已确认 `0e0XOb$L18ZBVYJiQJrQ1p` 与 `3bMoS7bIT8wBmjqRvdPunu` 整件为大白墙，`06wFwLoDD6ie5iCTnc_yad` 延后至最终统一选材。世界几何 `0.0 mm`、材料关系不变、正式 IFC 哈希不变；未决分界、产品系统与 3 个 C003 安装锚点写入 `wfin-open-issues.csv`。
- `pipeline:plum-existing-candidate`：只读登记 P-201 的 27 个 `IfcSanitaryTerminal` 和 P-202 的 49 个既有对象。类型层语义将其中 24 个保留为服务需求候选，另将 2 块已确认的 Geberit 115.770 冲水面板和 1 个 `DRAWER EQUIPMENT` 排除出需求端点；所有候选仍明确不是 `IfcDistributionPort`。当 IFC 中没有端口、系统或连接关系时报告 `data_missing`，不伪造连通性。两个 PVC110 产品继续只读识别为 6 个几何分支并以 C003 提交为世界几何冻结基线。
- `pipeline:plum-candidate`：在上述登记后生成 P-202 单页 500×400 mm SVG/PDF，以现有 Sanitary Plan 为底图，绘制 27 个洁具、3 个地漏、16 个排水对象和 3 个组合件的只读定位标记。标记不是连接口或粗装点；无 Port/System/连接关系时发布门继续阻断。
- `pipeline:plum-connection-candidate`：刷新当前 IFC 哈希的中心线证据，并把 24 个服务需求候选与 20 个排水几何分支按端点到洁具世界包围盒的最小距离只读配对。当前只有 3 个几何接触、3 个在 `150 mm` 内、18 个仅是较远的最近分支；该结果用于阻止误连，不把派生端点当正式接口，也不写 IFC。
- `pipeline:elec-existing-candidate`：只读盘点 E-301～E-304。当前输出 79 个灯具、11 个插座、8 个类型化设备和 9 个 C003 Proxy 移交对象；未实例化类型库与真实实例分开，零开关、零网络实例和零系统/端口关系保持为缺项，不自动写 IFC。
- `pipeline:elec-candidate`：在同一只读盘点中生成 E-301 灯具定位与 E-303 插座/设备定位 SVG/PDF。79 个灯具和 28 个插座/设备/Proxy 标签均通过范围、重复中心和碰撞门；图面明确不推断灯组、回路、功率、防水、控制或接口。
- `pipeline:elec-renovation-round1`：把用户确认的装修新增需求与开发商旧点位分层。当前只读输出 4 个床头灯平面候选、2 个新增插座候选、7 个有灯柜体供电协调区和 11 个厨房现有插座复核点。柜体协调区不是最终出线口；高度、回路、防水、设备功率及柜体开孔仍须在立面深化后反向复核。
- `pipeline:elec-renovation-review`：从现有正式 IFC 审核缓存重建装修用电 Blender 场景。青色=床头灯、绿色=新增插座、琥珀色=柜体供电协调区、红色=厨房现有插座复核；实心、真深度、`show_in_front=false`，不写 IFC。
- `pipeline:elec-room-program`：将 22 个 Space 编译为逐房间最小功能组矩阵，并从正式设备、已分类 Proxy、西门子洗衣机与热泵干衣机两个独立插头需求及哈希固定旧方案 A06 生成 17 个设备供电需求。当前矩阵包含 23 个控制功能组、26 个普通用电功能组、9 个弱电/数据功能组、9 个手动补充专用需求和 7 个柜体照明供电区。这些数量表示最小功能组，不是面板、插座孔、回路或端口数；APP-017 的协调净空中心也不等于两个侧向可检修插座或给排水接口位置。
- `pipeline:mep-positioning-source-audit`：以项目外层 `图纸/珠海-陈总(水电图)2025.12.23.pdf` 为当前厨房水电候选底稿，并用 SHA-256 固定 2025.5.29 与 2025.12.23 两版。两版均为 10 页；逐页渲染比较只有第 6 页变化，新版在油烟机插座处增加红圈。8 条来源要求登记 B/C/D 面和吧台的给排水、电器电位及预留电源；来源登记不等于全屋水电已完成。
- `pipeline:elec-positioning-candidate`：将厨房现有 11 个插座全部映射为来源支持的用途候选：2 个冰箱、1 个直饮机、1 个烤箱、1 个炉灶、1 个油烟机、2 个 H1200 墙体/导轨电源，以及 3 个 C 面净水器/备用插座组。9 个 Proxy 中两个精确 GlobalId 的 SJ45ZB24MC 已由用户确认为两台独立岛台洗碗机；其余形成 2 个冰箱体量、1 个垃圾处理器、3 个设备孔和 1 个排烟止回阀候选。洗碗机身份已关闭，但所有设备的功率、回路、给排水接口和安装锚点仍需审核。
- `pipeline/decisions/elec-design-rules.csv`：集中记录已确认的电气规则：只用实体有线开关；主卧/次卧各设 AP，客厅与书房作为开放空间共用 1 个路由器且不设 AP并保留有线回程；入户门与主卧门口设置客厅、书房、餐厅三组双控；入户照明总控排除冰箱、网络、安防和其他持续供电回路；普通插座、电视点和开关统一以面板底边距完成地面 300/600/1300 mm。所有天花设备必须与灯位共同排版；烟感优先位于房间几何中心或中心轴最近安全点，AP 可退到房间边缘的既有灯网空交点，偏位须登记冲突依据并经人审。卧室 AP 允许采用可下拆的磁吸齐平预埋底座，但辐射面不得被石膏板或金属遮挡。主卧、次卧、客厅烟感需求与小米烟雾报警器齐平预埋候选已登记，候选开孔/沉孔/预埋深度为 105/140/30 mm。厨房同时保留火灾探测和可燃气体探测两套独立需求；厨房油烟环境按感温优先建立协调需求，但最终探测类型由消防/设备方确认。小米燃气报警器候选尺寸为 94/120/43 mm，燃气公司确认型号前不得锁定专用开孔。探测器只能隐藏底座，探测口、指示灯和可拆卸主体必须外露。规则编译进只读协调候选，但不自动写 IFC。
- `pipeline:elec-control-network`：把入户门 M08、主卧门 M07 和已确认 Space 身份编译为 2 个门口控制协调区、2 个卧室 AP 房间区、1 个客厅—书房共享路由器协调区、3 个烟感房间区及 1 个厨房燃气报警器房间区，生成单页 500×400 mm SVG/PDF 与 JSON 门报告。门中心和 Space 包围盒中心只用于标明协调范围，显式标记为非最终 XY/Z 安装点；燃气型号审批状态单独作为机械门，不写 IFC。
- `pipeline:a106-ceiling-device-candidate`：把主卧、次卧烟感与吸顶 AP 及客厅烟感编译为 5 个精确候选。次卧烟感位于房间几何中心；客厅烟感距中心 `123 mm`；主卧中心被灯具包络占用，烟感沿中心轴移动到距中心 `522 mm` 的最近安全点。两个卧室 AP 采用房间边缘的既有灯位 X/Y 轴线空交点。XY 保持整数毫米，Z 轴直接服从既有天花下表面。逐点核验房间边界、梁、79 个既有灯具包络、RCP1 其他高位对象实际网格和既有天花下表面；烟感采用 500 mm 保守灯具净距。审核图保留 Bonsai 同批生成的 `Wall Plan-underlay.png`；生成前必须锁定正交 Drawing Camera、关闭三类缓存、恢复可读纹理路径，并用 `Wall Plan-source.json` 校验 IFC/SVG/PNG 哈希，否则硬失败。A-102 登记中的 13 面 `DEMOLISH` 墙继续排除。5/5 已知几何门通过。厨房火灾探测和燃气探测需求已经登记，但类型与点位仍待消防、燃气公司及设备方确认。正式 IFC 仍无完整装修后风口，故烟感距送风口 1500 mm/多孔送风 500 mm 保持 `unknown`，不写 IFC、不进入施工发布。
- `pipeline:scripts/elec_renovation_blender_review.py`：电气审核恢复全部正常 IFC 上下文后，按正式拆除登记精确隐藏 13 面 `DEMOLISH` 墙和默认关闭的 `A102_DEMOLITION_REFERENCE`；保持 Solid、真深度、X-Ray/Wireframe/`show_in_front` 关闭，家具为黄色，不写 IFC。
- `pipeline/scripts/mep_positioning_blender_review.py`：将 45 个既有给排水对象显示为蓝色、79 个灯具＋11 个插座＋8 个类型化设备显示为紫色，并将 3 个非服务洁具构件、1 个户型外孤立地漏和 9 个电气 Proxy 集中为 13 个红色审核对象。家具保持黄色；Solid、X-Ray/Wireframe/`show_in_front` 关闭，正式 IFC 不写入；派生审核缓存为 `build/mep-positioning/mep-positioning-review.blend`。
- `pipeline:mep-handover-reference`：用哈希固定 `矩阵纵横/(D1户型-115)建筑资料平面图.dxf`，并与从 `D户型交付竣工图.dwg` 转出的忽略文件机械比较；两者在户型范围内的 103 个水电参考点完全一致。流水线将 66 个电气点、11 个开关/控制点、20 个冷热水或马桶排水点及 6 个地漏转换至 IFC Grid 坐标，100/103 可唯一归属当前 Space，35 个电气点可读取原图高度。差异层将 77 个电气/控制点分为 1 个与当前插座近合、36 个既有墙参考候选和 40 个迁移/替换审核项；26 个给排水点分为 2 个近合、11 个协调区和 13 个仅交付/已迁移参考。SVG/PDF 和 `developer-handover-mep-review.blend` 都是改造前参考层，不是装修后设计，不写正式 IFC。
- `pipeline:int1-existing-candidate`：只读生成 I-501～I-504 的既有对象登记，保留 `89=70 固定＋19 活动` 家具口径，并把世界包围盒明确标记为协调证据而非加工尺寸。设备安装图、燃气现场实测、洁具粗装图与五金运动包络缺失时，INT1 施工发布门保持阻断。
- `pipeline:int1-handoff-candidate`：读取当前哈希的 C003 原点与整数几何锚点报告，只读分类精确移交 INT1 的 16 个 `IfcBuildingElementProxy`。当前结果为 3 个名称明确的台面、1 个名称明确的主卫管井包板、4 个 `CAD/Curve3D` 旧参考对象，以及由世界包络识别的 5 个水平薄板、2 个竖向薄板和 1 个通高木作体量候选；后 8 个对象仍未获得加工身份。只有 2 个对象存在整数几何点，3 个旧参考对象不能由 IfcOpenShell 生成实体网格。分类本身不授权重设原点、写语义或把包络当加工尺寸。
- `pipeline/scripts/int1_handoff_blender_review.py`：把上述 16 个对象集中到 Blender 审核层，只使用三种颜色：绿色 4 个名称已明确对象、橙色 8 个只有几何角色的对象、红色 4 个旧 CAD 参考对象。其他家具保持黄色；拆除墙和 HVAC 预览层隐藏；视图保持 Solid、真深度、X-Ray/Wireframe/`show_in_front` 关闭。脚本不保存 `.blend`，不写 IFC；当前派生审核缓存为 `build/int1/int1-handoff-review.blend`。
- `pipeline:int1-candidate`：在既有对象登记后生成 I-501～I-504 四张单页 500×400 mm SVG/PDF，129/129 个对象均以协调包络入图；图面与机械门共同声明这些包络不是加工、开孔或粗装尺寸。
- `pipeline:rcp1-existing-candidate`：只读登记 A-106/RCP1 的 117 个高位对象：2 个类型继承为 CEILING 的 Covering、79 个灯具、18 个 DCL Proxy、5 个已类型化 AC、7 个 AC Hole、5 个高位管段和 1 个厨房排烟止回阀 Proxy。每条记录包含世界包围盒、标高、Space 重叠、依据、置信度和人审状态；两个 C003 handoff 保持不动，缺失的正式风口、安全设备、检修口、系统和端口被明确阻断，不生成假 A-106。机械协调在可传 `0.1 mm` 容差下分类全部 6,786 对：6,474 对由 AABB 证明远离，312 对进入实际世界网格检测，最终 12 对接触、106 对相交、6,668 对分离。原 54 对逐对审核关系中，44 对天花构造、管线—天花、管线穿洞、风口嵌天花或已分离关系先行退出；其余 10 对管段/AC 相交已由哈希固定的旧 blend 证明为同一旧方案关系，因此当前逐对硬碰撞队列为 0。该关闭只接受旧几何作为改造底稿，不批准装修后 HVAC 路由或施工净空。
- `pipeline:m401-existing-candidate`：只读登记 M-401 的 28 个实际实例、3 个已分配 AC 类型定义和 5 个缺项 BLOCK。五条紫色管线明确标记为旧方案底稿，而不是装修后的最终管线；未赋类型的 HVAC mesh、Diffuser/止回阀 Proxy 不被自动解释为介质、系统、端口、风量、回路或连接。
- `pipeline:rcp1-legacy-base-audit`：通过 Blender 后台只读打开 SHA-256 `a8e0a94ef766667ad3749b96c158afdd538a330009b135e49ed461d1634c78ed` 的 `2504_lowpoly.blend`，对照当前正式 IFC。五条管线的独立分支数全部一致，组件包围尺寸最大差 `0.015289 mm`；当前十对管线/空调相交在旧底稿中 `10/10` 同样相交。旧文件同时存在厨房机位与 `1QBd...` 东侧额外空调对象；东侧固定机位中心与三台正式 AC 求得的旧/新平移基准最大轴差 `4.938 mm`，它尚无正式 GlobalId，但位置不再作为设计备选。脚本不保存 blend，不写 IFC。
- `pipeline:rcp1-hvac-remodel-candidate`：把 5 个正式 AC、旧 `.blend` 中 1 个位置已确认但无 GlobalId 的东侧 AC、7 个开发商既有空调洞和 5 个旧方案管道对象纳入同一只读候选。所有空调机位均作为固定输入，不再进行 A05/A06 选址比较。脚本机械识别 12 个独立管道几何分支，并利用现有世界网格协调报告逐台排序洞口与管道邻近关系；邻近或相交均不解释为端口连接。两个已确认风口同时作为局部 `-Y` 出风侧依据，Space 取样只生成直接服务房间候选并明确暴露客厅 R20 尚无直接设备或风口；跨房取样不得冒充已连接风管。输出只进入 `build/rcp1/`，在服务房间、送回风、厂家接管位置和冷凝水排放点确认前禁止写入正式 IFC。
  - 固定机位路径诊断采用最短平面路径与 `10 mm` 墙体包络审计：A05 到 R20 约 `2309.365 mm`、方向转角 `0°`，仅接触 `DEMOLISH` D11且不穿永久墙；A06 到 R20 约 `1399.338 mm`，但需转 `135.922°`、穿现状墙且尚缺正式 IFC 身份。该诊断只服务于固定机位后的送回风设计，不创建风管、风口或设备。
  - `DEMOLISH` 墙保留在正式 IFC 中供 A-102 出图；普通、RCP 和机电视图默认隐藏并从现存墙碰撞/净距判断中排除。需要核对拆除范围时，只开启半透明 `A102_DEMOLITION_REFERENCE` 参考层，保持真深度和 `show_in_front=0`。
- `pipeline:rcp1-blender-review`：清除旧的 A/B 路径选择标记，隐藏 13 面正式 `DEMOLISH` 源墙，并创建默认关闭的 `A102_DEMOLITION_REFERENCE` 半透明参考层。普通 HVAC 审查只显示建成墙；拆除参考开启时仍保持 Solid、真深度、X-Ray 关闭、Wireframe 关闭和 `show_in_front=0`。脚本只修改 Blender 审查状态，不写 IFC。
- `pipeline:rcp1-route-readiness-candidate`：以 6 个固定机位、7 个开发商既有洞口、哈希固定的旧 `.blend`、交付 DWG 和两张 HVAC 决策表为只读输入，生成允许共用洞口与多段路径的约束图。用户已确认 A02/A06 均服务餐厅、`A02→H03`、`A03→H04→H02`、H07 为冷凝水排放接口位置、H01 为室外机接口位置。H01/H07 的正式 IFC 实体仍保持 `IfcOpeningElement`，接口位置确认不等于实体重分类。旧管线分支拓扑与旧 `.blend` 一致，最大组件包围尺寸差 `0.015289 mm`；DWG 只作开发商设备/系统/预留洞参考。脚本不采用一对一就近配对，不移动设备、不新增洞口、不写 IFC。
- `pipeline:rcp1-route-blender-review`：按真实 Z 坐标绘制 `A02→H03` 与 `A03→H04→H02` 三段青色约束骨架，并分别标出 H01 室外机接口位置和 H07 冷凝水排放接口。图例明确骨架不是最终风管、冷媒管或厂家端口；所有对象保持 Solid、真深度、X-Ray 关闭、Wireframe 关闭和 `show_in_front=0`。脚本只修改 Blender 审查状态，不写 IFC。
- `pipeline/standards/hvac-route-authoring.md`：规定 IFC＋路线/弯点决策表为唯一可重建真源，Python 管理约束与候选编译，Geometry Nodes 只负责曲线圆角、截面和可视预览；移动设备或弯点不得通过 depsgraph 自动写 IFC。正式拓扑后续使用明确的风管/管段、管件、端口和系统关系。
- `pipeline/scripts/rcp1_hvac_constraint_preview.py`：在当前 Bonsai 会话安装 `RCP1 HVAC` 侧栏、7 个 IFC 锚点和 2 条 Geometry Nodes 路线预览。A02/A03 从设备局部 `+X` 厂家接管侧候选端面出发，先退出 `150 mm`，再生成只沿世界 X/Y/Z 的控制段；Geometry Nodes 以 `120 mm` 预览圆角显示折弯。移动设备/弯点只自动更新 Blender 曲线；“导出路线候选”把原 IFC 参考中心、服务端、世界主轴、有序锚点和全部正交控制点写到 `build/rcp1/hvac-route-preview-candidate.json`，不写 CSV、`.blend` 或 IFC。旧 HVAC 审查层自动隐藏，视图保持 Solid、真深度、X-Ray/Wireframe/`show_in_front` 关闭。
- `pipeline:rcp1-hvac-preview-audit`：用可传 `--tolerance-mm` 机械比较 Blender 导出的固定锚点、顺序和当前 IFC/路线约束图，并逐段验证只改变一个世界坐标轴。当前无临时弯点候选为 2 条路线、5 个固定锚点，参考锚点最大偏差 `0.000081 mm`；设备服务端相对包围盒中心最大偏移 `423.888 mm`，服务轴均为主轴，全部控制段正交，全部锚点按序落在路径上，Geometry Nodes 圆角节点存在，通过 `0.1 mm` 门。施工几何与正式 IFC 写入门继续关闭。
- `pipeline/decisions/rcp1-hvac-equipment-interface-evidence.csv`：登记当前三种 IFC 空调类型与日立官方安装资料的匹配边界。`RPIZ-22FSLN5QD/QDF(/P)` 的接管侧、管径与排水资料可作预览依据；`RPIZ-71FSLN5Q/P` 型号文本缺少 `D`，在完全匹配资料确认前保持待核对。
- `pipeline:rcp1-hvac-interface-candidate`：把厂家证据表与当前 IFC 的类型分配、M-401 哈希和本地厂家 PDF 哈希机械对照。当前 6 个固定机位中 5 个有正式 IFC 身份，A06 仍缺 GlobalId；A01–A04 共 4 台与 P02010Q 型号族匹配，A05 已由 P01530Q 的 `RPIZ-18~71*LN5Q*` 通配型号族覆盖关闭几何候选，但冷媒、排水和精确端口接口继续开放。全部 5 个既有设备的类型分配和本地 PDF 哈希通过，但 0 台具备已批准的精确端口坐标，正式 IFC 写入门保持关闭。
- `pipeline:wfin-candidate`：只读盘点 51 个有效 `CLADDING`，沿薄型垂直饰面的长轴与 Grid Space 边界机械求交，而不是仅用对象中心归属。当前意图展开为 52 个连续分段：24 个 Tadelakt、28 个大白墙；两个画面内对象已按用户确认覆盖 Space 自动推断并整件保留大白墙，仅 `06wFwLoDD6ie5iCTnc_yad` 延后到最终统一选材。该分段只覆盖既有 `CLADDING`，不冒充完整房间饰面；主卫干区返口、低墙顶面、门垛和跨区踢脚同样延后。命令生成对象表、分段表、500×400 mm SVG/PDF、项目专用意图 Pset 候选和 QA，不写正式 IFC。
...
- `pipeline:p0-semantics-batch-candidate`：把 A-104 与 A-105 上述内容从同一正式 IFC 原子合成，避免两个写入者或顺序覆盖。当前候选 SHA-256 为 `2ec94cce86eb764533bdb81b78a02b67ff2167694e851cf690ad37f74967856e`，重复生成逐字节一致；40 个 Tag、3 个门窗组、23 个审核 Pset 和 3 个参考面材料关联通过机械门，所有目标 placement 与世界几何变化 `0.0`。只有用户确认后才能复制为正式 IFC，并必须 reload/rebuild Blender。
- `pipeline:p0-semantics-postwrite-audit`：在 Bonsai 保存可能改变 STEP 序列化字节的情况下，把正式 IFC 与已批准候选按语义和几何比较；验证 Root、Fills/Voids、Tag、门窗组、Pset、材料、目标 ObjectPlacement 及 308 个门/窗/Opening/Covering/Slab 的世界几何，容差可传入且默认 `0.1 mm`。
- `pipeline:m072-package`：从当前正式 IFC、完整图纸台账与协调问题表生成 A-001 SVG/PDF 及人读 M072 协调包。机械门要求 A-102～A-105 生成报告与同一 IFC SHA 一致，五张候选 PDF 均为单页 `500×400 mm`且非空白，图号无重复，Release QA 的每个 BLOCK 都有责任方、目标任务和停止条件。本门允许已披露的后续问题存在，但不得把“已移交/已计划”写成“已解决”。
- `pipeline/scripts/p0_blender_review.py`：把 A-104 与 A-105 待审项一次性恢复为 Blender 真深度场景。红色仅表示 A-104 门窗待审；蓝色为已识别湿区地砖；橙色边界表示零厚度 FFL 参考面；紫色为 Aircrete 降板；家具保持黄色。18 个白色真深度箭头指向各湿区砖拟合面的下坡方向。普通墙、门、窗、洁具、排水和 Grid 保留作上下文，所有对象 `show_in_front=0`，Solid、X-Ray 关闭且不预选对象。3 个位于 `Z=0` 的零厚度参考面以真深度橙色 Solid 边界代理显示，源平面临时隐藏，避免遮住下方真实湿区地砖；不使用 Wireframe。
...
- `pipeline:a102-candidate`：从 Bonsai 重新刷新的 `drawings/Wall Plan.svg` 读取正式 IFC 的 101 面墙，生成 `drawings/Wall Plan-A102-candidate.svg` 与单页 PDF。灰色为 84 面现状墙、紫色为 4 面新建墙、红色斜线为 11 面“已拆记录”、紫红斜线为 2 面“计划拆除”；D01–D13 平面标签和侧栏对象表均以 `≈` 明示近似尺寸。出图门验证 13/13 源 SVG 几何、13/13 overlay、标签碰撞为 0、拆除墙包围盒最大差不超过 `0.1 mm`，并继承写后审计中原 284 个对象超差 0、受保护墙/门洞变化 `0.0 mm` 的证据。A-103 生成器同步排除全部 `DEMOLISH` 墙，避免拆除墙污染最终建成图。
- `pipeline:a104-candidate`：只读生成 A-104 SVG/PDF 和 `pipeline/decisions/a104-door-window-review.csv`。门采用 `M01–M08`、窗采用 `W01–W11`，按平面由北到南、同排由西到东确定；`D` 前缀已由 A-102 的拆除墙使用，跨图检查禁止复用。未人审前不写 IFC `Tag`。机械门验证 8 门、11 窗、5 个有宿主门、11 个有宿主窗、3 樘无宿主门，逐樘比较洞口宽高与 IFC 名义宽高，门限可传入且默认为 `0.1 mm`；同时检查编号重复、同宿主洞口重叠和生成标注碰撞。房间归属只作为基于当前 Space 几何的候选证据，不自动写回。Blender 审核由 `pipeline/scripts/a104_blender_review.py` 恢复完整几何上下文：普通门青色、窗蓝色、待审对象红色、家具黄色，Solid、X-Ray 关闭、`show_in_front=0`。
- `pipeline:ids-validate`：用 buildingSMART IfcTester 验证正式 IFC 和 P0 IDS，输出精简 JSON/Markdown。项目 Python 缺少 IfcTester 时，命令只复用本机 Bonsai 扩展中的纯 Python 包，不启动或修改 Blender；报告保留每项失败的精确 GlobalId，不嵌入冗长 STEP 实体。IDS 只检查信息要求，不替代 0.1 mm 几何关系和世界几何差分脚本。
...
- `pipeline:rcp1-handoff-origin-reset-candidate`：在 Blender 集中人审确认 16 个天花/灯槽边界与高低关系、并确认 `16Ey9Flj9BK9VRun$ozzjH` 是转角空调风口后，为书架跌级天花和转角风口生成 2 对象纯原点候选。书架对象采用已确认下表面的整数 Grid 点；风口采用现有端面的整数 Grid 点，距实际网格 `0.046702 mm`。候选必须保持全模型产品世界几何、schema、实体数和 Root GlobalId；不因此推断送回风、风量、系统、连接或检修要求。
- `pipeline:rcp1-handoff-origin-reset-postwrite`：只读比较正式 IFC 与获批的 2 对象候选；复核候选哈希、两个整数 placement、全模型产品世界几何、schema、实体数和 Root GlobalId。Bonsai 保存超时时以磁盘 IFC 为准，不凭超时信息判定失败。
