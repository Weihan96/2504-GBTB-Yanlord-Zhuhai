...
- `pipeline:int1-handoff-candidate`：读取当前哈希的 C003 原点与整数几何锚点报告，只读分类精确移交 INT1 的 16 个 `IfcBuildingElementProxy`。当前结果为 3 个名称明确的台面、1 个名称明确的主卫管井包板、4 个 `CAD/Curve3D` 旧参考对象和 8 个身份不足的通用木作 Proxy；只有 2 个对象存在整数几何点，3 个旧参考对象不能由 IfcOpenShell 生成实体网格。分类本身不授权重设原点、写语义或把包络当加工尺寸。
