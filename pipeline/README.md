...
- `pipeline:rcp1-route-readiness-candidate`：以 6 个固定机位和 7 个开发商既有洞口为只读输入，机械登记逐台洞口距离、直接服务 Space 候选和缺失端点。6 对 7 的全局一对一最小距离配对为 `A01→H05`、`A02→H02`、`A03→H04`、`A04→H06`、`A05→H07`、`A06→H03`，总距离 `2371.3 mm`，比次优组合少 `851.4 mm`；H01 保留为 A04 经 H06 向外延伸的服务链候选。该结果优于逐台贪心，但仍不是正式连接。正式 IFC 中 DistributionPort、System、室外机/立管接口均为 0，因此最终送回风、冷媒和冷凝水路线继续阻断。脚本不移动机位、不新增洞口、不把旧管线当最终路线，也不写 IFC。
- `pipeline:rcp1-route-blender-review`：在 `Z=3.08 m` 的独立审查层绘制 6 条青色“设备—洞口配对证据线”，红点标出未分配 H01，橙线标出 H06→H01 外部穿墙链待确认。图例明确这些线不是风管或冷媒管；所有对象保持 Solid、真深度、X-Ray 关闭、Wireframe 关闭和 `show_in_front=0`。脚本只修改 Blender 审查状态，不写 IFC。
