...
- `pipeline:rcp1-hvac-interface-candidate`：把厂家证据表与当前 IFC 的类型分配、M-401 哈希和本地厂家 PDF 哈希机械对照。当前 6 个固定机位中 5 个有正式 IFC 身份，A06 仍缺 GlobalId；A01–A04 共 4 台与 P02010Q 型号族匹配，A05 的 `RPIZ-71FSLN5Q/P` 型号文本仍不完全匹配。全部 5 个既有设备的类型分配和两个本地 PDF 哈希通过，但 0 台具备已批准的精确端口坐标，正式 IFC 写入门保持关闭。
