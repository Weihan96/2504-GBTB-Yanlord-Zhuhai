...
- `pipeline/decisions/elec-design-rules.csv`：集中记录已确认的电气规则：只用实体有线开关；主卧/次卧各设 AP，客厅与书房作为开放空间共用 1 个路由器且不设 AP并保留有线回程；入户门与主卧门口设置客厅、书房、餐厅三组双控；入户照明总控排除冰箱、网络、安防和其他持续供电回路；普通插座、电视点和开关统一以面板底边距完成地面 300/600/1300 mm。所有天花设备必须与灯位共同排版，优先服从空间主轴并采用共轴、等距、成组；偏位须登记冲突依据并经人审。卧室 AP 允许采用可下拆的磁吸齐平预埋底座，但辐射面不得被石膏板或金属遮挡；烟感只允许隐藏底座，进烟口、指示灯和探测器本体必须外露。规则编译进 E-302/E-304 候选，但不自动写 IFC。
- `pipeline:elec-control-network`：把入户门 M08、主卧门 M07 和 R09/R14/R20/R22 Space 的已确认身份编译为 2 个门口控制协调区、2 个卧室 AP 房间区和 1 个客厅—书房共享路由器协调区，生成单页 500×400 mm SVG/PDF 与 JSON 门报告。门中心和 Space 包围盒中心只用于标明协调范围，显式标记为非最终 XY/Z 安装点，不写 IFC。
