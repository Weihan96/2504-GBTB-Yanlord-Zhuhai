#!/usr/bin/env python3
"""Overlay one complete STREET-H Body proxy on the project sanitary plan."""
...
shared.PRODUCT_DIR = ROOT / "output/review/highpoly-types/street-h"
shared.CANDIDATE = shared.PRODUCT_DIR / "candidate-representations.json"
shared.GLOBAL_ID = "2ajpw0I9n1dBypfISg3ejX"
shared.OVERLAY_ID_PREFIX = "street-h"
shared.GENERATOR = "pipeline/scripts/render_street_h_project_context.py"
shared.PLAN_SOURCE = ROOT / "drawings/Sanitary Plan.svg"
shared.ALIGNMENT_MODE = "centre"
shared.BBOX_TOLERANCE_SVG_UNITS = 0.5
shared.CONTEXT_VIEWS = (
    ("plan", "plan", shared.PLAN_SOURCE, "project-context-sanitary-plan.svg", (10.0, 10.0), 0),
)
...
if __name__ == "__main__":
    ...
    for record in manifest["views"]:
        ...
        record["overlay"]["official_parent_family_cad_archived_as_rejected_reference"] = True
        record["overlay"]["official_parent_family_paths_used_as_project_representation"] = False
    manifest["official_cad_acquired"] = True
    manifest["official_parent_family_cad_archived_as_rejected_reference"] = True
    manifest["official_parent_family_paths_used_as_project_representation"] = False
    ...
    manifest["context_view_scope"] = {
        "included": ["plan"],
        "excluded": ["front", "side"],
        "reason": "The representative STREET-H GlobalId has one actual projection group in Sanitary Plan.svg and none in the native project elevation SVGs; project elevations are not invented."
    }
    manifest["project_projection_note"] = {
        "original_visible_projection_bbox_mm": [283.010559, 128.0],
        "complete_ifc_body_plan_bbox_mm": [300.0, 150.0],
