#!/usr/bin/env python3
"""Overlay Miami Soft I03 on its complete project plan and R20 side elevation."""
...
shared.PRODUCT_DIR = ROOT / "output/review/highpoly-types/miamisoft-i03"
shared.CANDIDATE = shared.PRODUCT_DIR / "candidate-representations.json"
shared.GLOBAL_ID = "1hdNtfQRPCGfHLfqzdp6w0"
shared.OVERLAY_ID_PREFIX = "miamisoft-i03"
shared.GENERATOR = "pipeline/scripts/render_miamisoft_i03_project_context.py"
...
shared.BBOX_TOLERANCE_SVG_UNITS = 0.25
...
def projection_group_points(source):
    ...
    if len(groups) != 1:
        raise RuntimeError(f"expected one complete Miami Soft I03 projection in {source}, found {len(groups)}")
    return groups[0]
...
if __name__ == "__main__":
    ...
    manifest["project_projection_note"] = {"official_nominal_length_diameter_mm": [1700.0, 270.0], "ifc_body_local_xyz_mm": [1711.749878, 274.261993, 272.80899], "project_ifc_description": "Roll 108 x Ø27 cm", "description_conflict": "108 cm is the adjacent official I01 length; I03 type code and Body geometry match official 170 cm I03", "interpretation": "The complete geometry-derived proxy is overlaid at the drawing's native 1:50 scale and centre-aligned; no axis is stretched to force the nominal roll envelope or the stale 108 cm Description.", "geometry_stretched": False}
