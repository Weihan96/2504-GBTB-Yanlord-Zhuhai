#!/usr/bin/env python3
"""Load one Duofix in Bonsai, add local-axis cameras, and execute renders."""
...
PRODUCT_DIR = ROOT / os.environ.get("HIGHPOLY_PRODUCT_DIR", "output/review/highpoly-types/geberit-duofix-sigma-224-212")
SOURCE = PRODUCT_DIR / os.environ.get("HIGHPOLY_ISOLATED_IFC", "Geberit-Duofix-Sigma-224-212-bonsai-isolated.ifc")
BLEND = PRODUCT_DIR / os.environ.get("HIGHPOLY_BLEND", "Geberit-Duofix-Sigma-224-212-bonsai-review.blend")
MANIFEST = PRODUCT_DIR / "bonsai-review-manifest.json"
GLOBAL_ID = os.environ.get("HIGHPOLY_GLOBAL_ID", "3hgNkx97vCTOC2eewCpMNk")
CAMERA_PREFIX = os.environ.get("HIGHPOLY_CAMERA_PREFIX", "GEBERIT_DUOFIX_224_212")
PRODUCT_LABEL = os.environ.get("HIGHPOLY_PRODUCT_LABEL", "Duofix")
GENERATOR = os.environ.get("HIGHPOLY_GENERATOR", "pipeline/scripts/render_geberit_duofix_sigma_224_212_bonsai_review.py")
...
FRAMING_MARGIN_FACTOR = 1.18
ORTHO_SCALE_QUANTIZATION_SAFETY_FACTOR = 1.000001
FRONT_DIRECTION_SIGN = float(os.environ.get("HIGHPOLY_FRONT_DIRECTION_SIGN", "-1"))
if FRONT_DIRECTION_SIGN not in {-1.0, 1.0}:
    raise RuntimeError("HIGHPOLY_FRONT_DIRECTION_SIGN must be -1 or 1")
...
def render_cameras(obj):
    ...
    front_direction = axis_y * FRONT_DIRECTION_SIGN
    jobs = [
        ("PLAN", axis_z),
        ("FRONT", front_direction),
        ("SIDE", axis_x),
        ("ISO", (-axis_x + front_direction + axis_z).normalized()),
    ]
    results = []
    for label, direction in jobs:
        camera_data = bpy.data.cameras.new(f"{CAMERA_PREFIX}_CAM_{label}")
        camera = bpy.data.objects.new(f"{CAMERA_PREFIX}_CAM_{label}", camera_data)
        ...
        projected_width = axis_extent(corners, center, camera_right)
        projected_height = axis_extent(corners, center, camera_up)
        image_aspect = scene.render.resolution_x / scene.render.resolution_y
        camera.data.ortho_scale = (
            max(projected_width, projected_height * image_aspect)
            * FRAMING_MARGIN_FACTOR
            * ORTHO_SCALE_QUANTIZATION_SAFETY_FACTOR
        )
        scene.camera = camera
        ...
        results.append({
            ...
            "camera_ortho_scale_m": round(camera.data.ortho_scale, 12),
            "projected_width_m": round(projected_width, 12),
            "projected_height_m": round(projected_height, 12),
            "image_aspect": round(image_aspect, 12),
            "framing_margin_factor": FRAMING_MARGIN_FACTOR,
            "ortho_scale_quantization_safety_factor": ORTHO_SCALE_QUANTIZATION_SAFETY_FACTOR,
...
if product is None:
    raise RuntimeError(f"{PRODUCT_LABEL} representative is missing from Bonsai session")
obj = tool.Ifc.get_object(product)
if obj is None:
    raise RuntimeError(f"Bonsai did not create the {PRODUCT_LABEL} Blender object")
...
if (
    representation.ContextOfItems.ContextIdentifier != "Body"
    or representation.ContextOfItems.TargetView != "MODEL_VIEW"
):
    raise RuntimeError("camera renders must use the actual MODEL_VIEW IFC Body representation")
...
payload = {
    ...
    "bonsai_session": {
        ...
        "ifc_target_view": representation.ContextOfItems.TargetView,
        "front_camera_local_y_sign": FRONT_DIRECTION_SIGN,
