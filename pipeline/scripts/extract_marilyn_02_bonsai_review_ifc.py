#!/usr/bin/env python3
"""Extract one Baxter Marilyn 02 pouf IFC Body for Bonsai camera review."""
...
OUTPUT = ROOT / "output/review/highpoly-types/marilyn-02/Baxter-Marilyn-02-pouf-bonsai-isolated.ifc"
GLOBAL_ID = "1THxa7p7n97w$wtLn4THjz"
...
if product is None:
    raise RuntimeError("Marilyn 02 representative was not extracted")
...
if not any(representation.ContextOfItems.TargetView == "MODEL_VIEW" for representation in body_representations):
    raise RuntimeError("isolated Marilyn 02 IFC lost its actual MODEL_VIEW Body representation")
