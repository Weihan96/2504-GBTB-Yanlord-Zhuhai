#!/usr/bin/env python3
"""Overlay exact Geberit Sigma01 native-DWG views on project-context SVGs."""
...
PRODUCT_DIR = ROOT / "output/review/highpoly-types/geberit-115-770"
...
REPRESENTATIVE = "2gFgcOYEXEaQWAzcKulTFt"
INSTANCES = [REPRESENTATIVE, "2lDPsdQevFSfeOThtjSlPG"]
NUMBER = re.compile(r"[-+]?(?:\d*\.\d+|\d+)(?:[eE][-+]?\d+)?")
BLUE = "#1677c8"
PROJECT_SCALE_SVG_UNITS_PER_MM = 0.02
BBOX_TOLERANCE_SVG_UNITS = 0.2
...
def add_overlays(source: Path, target: Path, overlays: list[dict]):
    ...
    for overlay in overlays:
        path = overlay["path"]
        groups.append(f'''<g id="geberit-115-770-{overlay["view"]}-{overlay["guid"]}" class="official-native-dwg project-context-overlay" data-source-kind="native_dwg" data-article-number="115.770.11.5" data-native-dwg-code="{overlay["code"]}" data-source-sha256="{overlay["sha256"]}" data-ifc-guid="{overlay["guid"]}" fill="none" stroke-linecap="round" stroke-linejoin="round">
...
def write_png_preview(source: Path, target: Path):
    ...
    with tempfile.TemporaryDirectory(prefix="geberit-115770-context-preview-") as temporary:
...
def main():
    ...
    if linework.get("source_kind") != "native_dwg" or linework.get("article_number") != "115.770.11.5":
    ...
    manifest = {
        "schema_version": 1,
        "generator": "pipeline/scripts/render_geberit_115_770_project_context.py",
        ...
        "project_ifc_body_and_official_dwg_both_unscaled": True,
        "documented_legacy_body_size_tolerance_mm": 10.0,
