#!/usr/bin/env python3
"""Approval-gated derived IFC writer for Baxter Miami Soft F03."""
...
SCOPE = "exact manufacturer family, F03 pouf model and nominal dimensions; authenticated native 2D/3D/BIM not acquired; not a project shop drawing and not official CAD geometry"
...
MEASUREMENT_SVG = "https://productsbook.baxter.it/models/measurements/MIAMSOPSF03C.svg"
PRODUCT_DIR = ROOT / "output/review/highpoly-types/miamisoft-f03"
...
MEASUREMENT_SVG_ARCHIVE = PRODUCT_DIR / "official-source/MIAMSOPSF03C.svg"
EXPECTED_PATH_COUNTS = {"plan": 29, "front": 20, "side": 28}

shared.REPRESENTATIVE_GLOBAL_ID = "1y3lLYK$H9fPbuh1R1vAKX"
shared.IFC_TYPE_NAME = "MiamiSoft F03"
shared.PROFILE_KEY = "miamisoft-f03"
shared.SCOPE = SCOPE
shared.REPRESENTATIONS = {
    "plan": ("MiamiSoftF03Plan", "PLAN_VIEW"),
    "front": ("MiamiSoftF03Front", "ELEVATION_VIEW"),
    "side": ("MiamiSoftF03Side", "ELEVATION_VIEW"),
}
...
shared.DEFAULT_APPROVAL = ROOT / "pipeline/decisions/miamisoft-f03-drawing-approval.json"
shared.PSET_NAME = "Pset_MiamiSoftF03DrawingSource"
shared.DOCUMENT_ID_PREFIX = "BAXTER-MIAMI-SOFT-F03-"
...
def candidate_paths(candidate: dict, access: dict) -> dict:
    if (
        ...
        or candidate.get("article_number") != "Baxter Miami Soft F03 pouf"
        ...
        raise RuntimeError("pending Miami Soft F03 candidate source gate failed")
    ...
    if (
        access.get("resolved_variant") != "F03 - pouf - 170 x 108 h40 cm"
        ...
        or identity.get("module_status") != "confirmed_F03_pouf"
        or access.get("dimension_cross_check", {}).get("pass") is not True
    ):
        raise RuntimeError("Miami Soft F03 official-source access record gate failed")
    paths = {}
    for view in shared.REQUIRED_VIEWS:
        ...
        if item.get("source_kind") != shared.SOURCE_KIND or item.get("official_cad_paths_mm") != []:
            raise RuntimeError(f"Miami Soft F03 {view} must contain no official CAD paths")
        paths[view] = item.get("proxy_paths_mm", [])
        if len(paths[view]) != EXPECTED_PATH_COUNTS[view]:
            raise RuntimeError(f"Miami Soft F03 {view} geometry-derived path count drifted")
    return paths


def add_document_associations(model, product, product_type, access_path: Path):
    documents = (
        ("BAXTER-MIAMI-SOFT-F03-OFFICIAL-PRODUCT-PAGE", PRODUCT_PAGE, "Baxter official Miami Soft product page", "Exact family, designer, F03 pouf model and authenticated native-file access boundary; not drawing geometry"),
        ("BAXTER-MIAMI-SOFT-F03-OFFICIAL-PRODUCT-PAGE-ARCHIVE", relative(PRODUCT_PAGE_ARCHIVE), "Archived Baxter Miami Soft product page", f"SHA-256 {sha256(PRODUCT_PAGE_ARCHIVE)}; identity and access evidence only"),
        ("BAXTER-MIAMI-SOFT-F03-OFFICIAL-TECHNICAL-SHEET", TECHNICAL_SHEET, "Baxter official Miami Soft vector technical sheet", "Page 7 confirms F03 pouf and 170 x 108 h40 cm; evidence only, not representation linework"),
        ("BAXTER-MIAMI-SOFT-F03-OFFICIAL-TECHNICAL-SHEET-ARCHIVE", relative(TECHNICAL_SHEET_ARCHIVE), "Archived Baxter Miami Soft technical sheet", f"SHA-256 {sha256(TECHNICAL_SHEET_ARCHIVE)}; vector PDF not used as CAD geometry"),
        ("BAXTER-MIAMI-SOFT-F03-OFFICIAL-MEASUREMENT-SVG", MEASUREMENT_SVG, "Baxter official Miami Soft F03 measurement SVG", "Exact F03 three-view dimension graphic; evidence only, not native DWG/DXF representation geometry"),
        ("BAXTER-MIAMI-SOFT-F03-OFFICIAL-MEASUREMENT-SVG-ARCHIVE", relative(MEASUREMENT_SVG_ARCHIVE), "Archived Baxter Miami Soft F03 measurement SVG", f"SHA-256 {sha256(MEASUREMENT_SVG_ARCHIVE)}; official vector reference not used as CAD geometry"),
        ("BAXTER-MIAMI-SOFT-F03-SOURCE-ACCESS-RECORD", relative(access_path), "Baxter Miami Soft F03 source access record", f"SHA-256 {sha256(access_path)}; native 2D/3D/BIM login required; no third-party CAD used"),
    )
    identifiers = []
...
def add_source_pset(model, product, product_type, approval, manifest_hash, candidate_path, access_path):
    ...
    values = {
        ...
        "ModelCode": "F03",
        "Module": "F03 pouf",
        "IFCTypeDescription": "Pouf 170 x 108 h40 cm",
        ...
        "ApprovedRepresentationIdentifiers": "MiamiSoftF03Plan;MiamiSoftF03Front;MiamiSoftF03Side",
    ...
    pset = model.create_entity(
        ...
        Description="Mechanically verifiable geometry-derived source, exact F03 identity evidence and human approval",
    ...
    model.create_entity(
        ...
        GlobalId=ifcopenshell.guid.new(),
        OwnerHistory=product.OwnerHistory,
        Name="Miami Soft F03 drawing source and model properties",
        Description=None,
        RelatedObjects=[product, product_type],
