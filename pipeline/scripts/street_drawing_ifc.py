#!/usr/bin/env python3
"""Approval-gated derived IFC writer for the domestic-custom STREET top."""
...
SCOPE = "official antoniolupi Street family CAD evidence for a domestic-custom project top; no exact 1000 x 470 x 250 mm official configuration is present and no project shop drawing is claimed"
...
DXF_AUDIT = PRODUCT_DIR / "official-native-dxf-configuration-audit.json"
EXPECTED_PATH_COUNTS = {"plan": 3, "front": 1, "side": 2}

shared.REPRESENTATIVE_GLOBAL_ID = "1FgLPMw$5B4wBH2ySMkXE1"
shared.IFC_TYPE_NAME = "STREET"
shared.PROFILE_KEY = "street"
shared.SCOPE = SCOPE
shared.REPRESENTATIONS = {
    "plan": ("StreetPlan", "PLAN_VIEW"),
    "front": ("StreetFront", "ELEVATION_VIEW"),
    "side": ("StreetSide", "ELEVATION_VIEW"),
}
...
shared.DEFAULT_APPROVAL = ROOT / "pipeline/decisions/street-drawing-approval.json"
shared.PSET_NAME = "Pset_StreetDrawingSource"
shared.DOCUMENT_ID_PREFIX = "ANTONIOLUPI-STREET-"
...
def candidate_paths(candidate: dict, access: dict) -> dict:
    if (
        ...
        raise RuntimeError("pending STREET candidate source gate failed")
    ...
    if (
        ...
        or cad.get("exact_family_located") is not True
        or cad.get("exact_project_configuration_match") is not False
        or cad.get("official_dxf_paths_used_as_project_representation") is not False
        or cad.get("native_dxf_sha256") != sha256(NATIVE_DXF)
        or cad.get("native_dxf_configuration_audit_sha256") != sha256(DXF_AUDIT)
        ...
        raise RuntimeError("STREET official family/custom-configuration source gate failed")
    paths = {}
    for view in shared.REQUIRED_VIEWS:
        ...
        if item.get("source_kind") != shared.SOURCE_KIND or item.get("official_cad_paths_mm") != []:
            raise RuntimeError(f"STREET {view} linework must remain geometry-derived with no official CAD paths")
        paths[view] = item.get("proxy_paths_mm", [])
        if len(paths[view]) != EXPECTED_PATH_COUNTS[view]:
            raise RuntimeError(f"STREET {view} geometry-derived path-count gate failed")
    return paths
...
def add_document_associations(model, product, product_type, access_path):
    documents = (
        (
            ...
            "Manufacturer, Street family and download-surface evidence only; not STREET representation geometry",
        ),
        (
        ...
        (
            ...
            f"SHA-256 {sha256(CAD_ZIP)}; official family CAD, exact domestic-custom project configuration absent",
        ),
        (
            "ANTONIOLUPI-STREET-OFFICIAL-NATIVE-DXF",
            relative(NATIVE_DXF),
            "antoniolupi official AL_Street native DXF",
            f"SHA-256 {sha256(NATIVE_DXF)}; contains standard clusters but no exact 1000 x 470 mm plan cluster",
        ),
        (
            "ANTONIOLUPI-STREET-OFFICIAL-TECHNICAL-PDF",
            relative(TECHNICAL_PDF),
            "antoniolupi official Street technical data sheet",
            f"SHA-256 {sha256(TECHNICAL_PDF)}; official Street family dimensions and modular configuration boundaries",
        ),
        (
        ...
        (
            ...
            relative(DXF_AUDIT),
            "STREET native DXF configuration audit",
            f"SHA-256 {sha256(DXF_AUDIT)}; 1080 x 400 and 1080 x 470 standard clusters are rejected as the 1000 x 470 project representation",
        ),
        (
            "ANTONIOLUPI-STREET-SOURCE-ACCESS-RECORD",
            relative(access_path),
            "STREET manufacturer-source access record",
            f"SHA-256 {sha256(access_path)}; exact domestic-custom CAD absent and no third-party CAD used",
        ),
    )
    identifiers = []
...
def add_source_pset(model, product, product_type, approval, manifest_hash, candidate_path, access_path):
    ...
    values = {
        ...
        "Family": "Street",
        "ProjectTypeCode": "STREET",
        "IFCTypeDescription": "antoniolupi street240 prof. 40 + street4054 prof. 40",
        ...
        "ExactProjectConfigurationCadMatch": "false",
        "ProjectProcurementScope": "domestic_custom_not_exact_official_standard_article",
        "RejectedIfcDescriptionCluster": "STREET240 prof. 40 + STREET4054 prof. 40 = 1080 x 400 x 250 mm",
        "RejectedNearestDepthCluster": "STREET147 prof. 47 + STREET4754 prof. 40 = 1080 x 470 x 250 mm",
        "Exact1000x470OfficialPlanClusterPresent": "false",
        "ProjectStreetBodyBoundsMm": "1000 x 470 x 250",
        "EvidenceScope": SCOPE,
        ...
        "ApprovedRepresentationIdentifiers": "StreetPlan;StreetFront;StreetSide",
        ...
        "OfficialDxfConfigurationAudit": relative(DXF_AUDIT),
        "OfficialDxfConfigurationAuditSha256": sha256(DXF_AUDIT),
        "ConfigurationCrossCheckStatus": str(access["configuration_cross_check"]["pass"]).lower(),
    ...
    pset = model.create_entity(
        ...
        Description="Mechanically verifiable geometry-derived STREET drawing source and human approval",
    ...
    model.create_entity(
        ...
        GlobalId=ifcopenshell.guid.new(),
        OwnerHistory=product.OwnerHistory,
        Name="STREET drawing source properties",
        Description=None,
        RelatedObjects=[product, product_type],
