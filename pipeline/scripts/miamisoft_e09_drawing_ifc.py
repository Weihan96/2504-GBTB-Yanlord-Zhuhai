#!/usr/bin/env python3
"""Approval-gated derived IFC writer for Baxter Miami Soft E09."""
...
SCOPE = "exact manufacturer family, E09 handed variant and nominal dimensions; authenticated native 2D/3D/BIM not acquired; not a project shop drawing and not official CAD geometry"
...
MEASUREMENT_SVG = "https://productsbook.baxter.it/models/measurements/MIAMSOESE09D.svg"
...
MEASUREMENT_SVG_ARCHIVE = PRODUCT_DIR / "official-source/MIAMSOESE09D.svg"
EXPECTED_PATH_COUNTS = {"plan": 25, "front": 28, "side": 35}

shared.REPRESENTATIVE_GLOBAL_ID = "3osoWufdD1mhDDAM6lcix4"
shared.IFC_TYPE_NAME = "MiamiSoft E09"
shared.PROFILE_KEY = "miamisoft-e09"
shared.SCOPE = SCOPE
shared.REPRESENTATIONS = {
    "plan": ("MiamiSoftE09Plan", "PLAN_VIEW"),
    "front": ("MiamiSoftE09Front", "ELEVATION_VIEW"),
    "side": ("MiamiSoftE09Side", "ELEVATION_VIEW"),
}
...
shared.DEFAULT_APPROVAL = ROOT / "pipeline/decisions/miamisoft-e09-drawing-approval.json"
shared.PSET_NAME = "Pset_MiamiSoftE09DrawingSource"
shared.DOCUMENT_ID_PREFIX = "BAXTER-MIAMI-SOFT-E09-"
...
def candidate_paths(candidate: dict, access: dict) -> dict:
    if (
        ...
        or candidate.get("article_number") != "Baxter Miami Soft E09 dx/r"
        ...
        raise RuntimeError("pending Miami Soft E09 candidate source gate failed")
    ...
    if (
        access.get("resolved_variant") != "E09 - dx/r - right terminal module - 130 x 108 h70/80 cm"
        ...
        or identity.get("handed_variant_status") != "confirmed_E09_dx_r_right_terminal"
        or access.get("dimension_cross_check", {}).get("pass") is not True
    ):
        raise RuntimeError("Miami Soft E09 official-source access record gate failed")
    paths = {}
    for view in shared.REQUIRED_VIEWS:
        ...
        if item.get("source_kind") != shared.SOURCE_KIND or item.get("official_cad_paths_mm") != []:
            raise RuntimeError(f"Miami Soft E09 {view} must contain no official CAD paths")
        paths[view] = item.get("proxy_paths_mm", [])
        if len(paths[view]) != EXPECTED_PATH_COUNTS[view]:
            raise RuntimeError(f"Miami Soft E09 {view} geometry-derived path count drifted")
    return paths
...
def add_document_associations(model, product, product_type, access_path: Path):
    documents = (
        (
            "BAXTER-MIAMI-SOFT-E09-OFFICIAL-PRODUCT-PAGE",
            PRODUCT_PAGE,
            "Baxter official Miami Soft product page",
            "Exact family, designer, E09 variant and authenticated native-file access boundary; not drawing geometry",
        ),
        (
            "BAXTER-MIAMI-SOFT-E09-OFFICIAL-PRODUCT-PAGE-ARCHIVE",
            relative(PRODUCT_PAGE_ARCHIVE),
            "Archived Baxter Miami Soft product page",
            f"SHA-256 {sha256(PRODUCT_PAGE_ARCHIVE)}; identity and access evidence only",
        ),
        (
            "BAXTER-MIAMI-SOFT-E09-OFFICIAL-TECHNICAL-SHEET",
            TECHNICAL_SHEET,
            "Baxter official Miami Soft vector technical sheet",
            "Page 6 confirms E09 dx/r and 130 x 108 h70/80 cm; evidence only, not representation linework",
        ),
        (
            "BAXTER-MIAMI-SOFT-E09-OFFICIAL-TECHNICAL-SHEET-ARCHIVE",
            relative(TECHNICAL_SHEET_ARCHIVE),
            "Archived Baxter Miami Soft technical sheet",
            f"SHA-256 {sha256(TECHNICAL_SHEET_ARCHIVE)}; vector PDF not used as CAD geometry",
        ),
        (
            "BAXTER-MIAMI-SOFT-E09-OFFICIAL-MEASUREMENT-SVG",
            MEASUREMENT_SVG,
            "Baxter official Miami Soft E09 measurement SVG",
            "Exact E09 three-view dimension graphic; evidence only, not native DWG/DXF representation geometry",
        ),
        (
            "BAXTER-MIAMI-SOFT-E09-OFFICIAL-MEASUREMENT-SVG-ARCHIVE",
            relative(MEASUREMENT_SVG_ARCHIVE),
            "Archived Baxter Miami Soft E09 measurement SVG",
            f"SHA-256 {sha256(MEASUREMENT_SVG_ARCHIVE)}; official vector reference not used as CAD geometry",
        ),
        (
            "BAXTER-MIAMI-SOFT-E09-SOURCE-ACCESS-RECORD",
...
def add_source_pset(model, product, product_type, approval, manifest_hash, candidate_path, access_path):
    ...
    values = {
        ...
        "ModelCode": "E09",
        "HandedVariant": "dx/r right terminal module",
        "IFCTypeDescription": "R terminal module 130 x 108 h70/80 cm",
        ...
        "ApprovedRepresentationIdentifiers": "MiamiSoftE09Plan;MiamiSoftE09Front;MiamiSoftE09Side",
    ...
    pset = model.create_entity(
        ...
        Description="Mechanically verifiable geometry-derived source, exact E09 identity evidence and human approval",
    ...
    model.create_entity(
        ...
        GlobalId=ifcopenshell.guid.new(),
        OwnerHistory=product.OwnerHistory,
        Name="Miami Soft E09 drawing source and variant properties",
        Description=None,
        RelatedObjects=[product, product_type],
