#!/usr/bin/env python3
"""Approval-gated derived IFC writer for Baxter Miami Soft E07."""
...
SCOPE = "exact manufacturer family, E07 handed variant and nominal dimensions; authenticated native 2D/3D/BIM not acquired; not a project shop drawing and not official CAD geometry"
...
MEASUREMENT_SVG = "https://productsbook.baxter.it/models/measurements/MIAMSOESE07S.svg"
PRODUCT_DIR = ROOT / "output/review/highpoly-types/miamisoft-e07"
...
MEASUREMENT_SVG_ARCHIVE = PRODUCT_DIR / "official-source/MIAMSOESE07S.svg"
EXPECTED_PATH_COUNTS = {"plan": 25, "front": 31, "side": 33}

shared.REPRESENTATIVE_GLOBAL_ID = "1G7iO6CfP64PjummX$qTbt"
shared.IFC_TYPE_NAME = "MiamiSoft E07"
shared.PROFILE_KEY = "miamisoft-e07"
shared.SCOPE = SCOPE
shared.REPRESENTATIONS = {
    "plan": ("MiamiSoftE07Plan", "PLAN_VIEW"),
    "front": ("MiamiSoftE07Front", "ELEVATION_VIEW"),
    "side": ("MiamiSoftE07Side", "ELEVATION_VIEW"),
}
...
shared.DEFAULT_APPROVAL = ROOT / "pipeline/decisions/miamisoft-e07-drawing-approval.json"
shared.PSET_NAME = "Pset_MiamiSoftE07DrawingSource"
shared.DOCUMENT_ID_PREFIX = "BAXTER-MIAMI-SOFT-E07-"
...
def candidate_paths(candidate: dict, access: dict) -> dict:
    if (
        ...
        or candidate.get("article_number") != "Baxter Miami Soft E07 sx/l"
        ...
        raise RuntimeError("pending Miami Soft E07 candidate source gate failed")
    ...
    if (
        access.get("resolved_variant") != "E07 - sx/l - left dormeuse - 150 x 170 h70/80 cm"
        ...
        or identity.get("handed_variant_status") != "confirmed_E07_sx_l_left_dormeuse"
        or access.get("dimension_cross_check", {}).get("pass") is not True
    ):
        raise RuntimeError("Miami Soft E07 official-source access record gate failed")
    paths = {}
    for view in shared.REQUIRED_VIEWS:
        ...
        if item.get("source_kind") != shared.SOURCE_KIND or item.get("official_cad_paths_mm") != []:
            raise RuntimeError(f"Miami Soft E07 {view} must contain no official CAD paths")
        paths[view] = item.get("proxy_paths_mm", [])
        if len(paths[view]) != EXPECTED_PATH_COUNTS[view]:
            raise RuntimeError(f"Miami Soft E07 {view} geometry-derived path count drifted")
    return paths


def add_document_associations(model, product, product_type, access_path: Path):
    documents = (
        ("BAXTER-MIAMI-SOFT-E07-OFFICIAL-PRODUCT-PAGE", PRODUCT_PAGE, "Baxter official Miami Soft product page", "Exact family, designer, E07 variant and authenticated native-file access boundary; not drawing geometry"),
        ("BAXTER-MIAMI-SOFT-E07-OFFICIAL-PRODUCT-PAGE-ARCHIVE", relative(PRODUCT_PAGE_ARCHIVE), "Archived Baxter Miami Soft product page", f"SHA-256 {sha256(PRODUCT_PAGE_ARCHIVE)}; identity and access evidence only"),
        ("BAXTER-MIAMI-SOFT-E07-OFFICIAL-TECHNICAL-SHEET", TECHNICAL_SHEET, "Baxter official Miami Soft vector technical sheet", "Page 7 confirms E07 sx/l and 150 x 170 h70/80 cm; evidence only, not representation linework"),
        ("BAXTER-MIAMI-SOFT-E07-OFFICIAL-TECHNICAL-SHEET-ARCHIVE", relative(TECHNICAL_SHEET_ARCHIVE), "Archived Baxter Miami Soft technical sheet", f"SHA-256 {sha256(TECHNICAL_SHEET_ARCHIVE)}; vector PDF not used as CAD geometry"),
        ("BAXTER-MIAMI-SOFT-E07-OFFICIAL-MEASUREMENT-SVG", MEASUREMENT_SVG, "Baxter official Miami Soft E07 measurement SVG", "Exact E07 three-view dimension graphic; evidence only, not native DWG/DXF representation geometry"),
        ("BAXTER-MIAMI-SOFT-E07-OFFICIAL-MEASUREMENT-SVG-ARCHIVE", relative(MEASUREMENT_SVG_ARCHIVE), "Archived Baxter Miami Soft E07 measurement SVG", f"SHA-256 {sha256(MEASUREMENT_SVG_ARCHIVE)}; official vector reference not used as CAD geometry"),
        ("BAXTER-MIAMI-SOFT-E07-SOURCE-ACCESS-RECORD", relative(access_path), "Baxter Miami Soft E07 source access record", f"SHA-256 {sha256(access_path)}; native 2D/3D/BIM login required; no third-party CAD used"),
    )
    identifiers = []
...
def add_source_pset(model, product, product_type, approval, manifest_hash, candidate_path, access_path):
    ...
    values = {
        ...
        "ModelCode": "E07",
        "HandedVariant": "sx/l left dormeuse",
        "IFCTypeDescription": "L dormeuse 150 x 170 h70/80 cm",
        ...
        "ApprovedRepresentationIdentifiers": "MiamiSoftE07Plan;MiamiSoftE07Front;MiamiSoftE07Side",
        ...
        "OfficialNominalWidthDepthHeightMm": json.dumps(dimensions["official_nominal_width_depth_height_mm"]),
        "ProjectIfcBodyLocalXYZMm": json.dumps(dimensions["project_ifc_body_local_xyz_mm"]),
        "BodyMinusOfficialNominalMm": json.dumps(dimensions["body_minus_official_nominal_mm"]),
    ...
    pset = model.create_entity(
        ...
        Description="Mechanically verifiable geometry-derived source, exact E07 identity evidence and human approval",
    ...
    model.create_entity(
        ...
        GlobalId=ifcopenshell.guid.new(),
        OwnerHistory=product.OwnerHistory,
        Name="Miami Soft E07 drawing source and variant properties",
        Description=None,
        RelatedObjects=[product, product_type],
