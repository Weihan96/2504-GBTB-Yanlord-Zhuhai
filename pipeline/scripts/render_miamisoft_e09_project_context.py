#!/usr/bin/env python3
"""Overlay Miami Soft E09 on its complete project plan and R20 side elevation."""
...
shared.PRODUCT_DIR = ROOT / "output/review/highpoly-types/miamisoft-e09"
shared.CANDIDATE = shared.PRODUCT_DIR / "candidate-representations.json"
shared.GLOBAL_ID = "3osoWufdD1mhDDAM6lcix4"
shared.OVERLAY_ID_PREFIX = "miamisoft-e09"
shared.GENERATOR = "pipeline/scripts/render_miamisoft_e09_project_context.py"
...
shared.BBOX_TOLERANCE_SVG_UNITS = 1.5
...
def projection_group_points(source):
    ...
    if len(groups) != 1:
        raise RuntimeError(f"expected one complete Miami Soft projection in {source}, found {len(groups)}")
    return groups[0]
...
if __name__ == "__main__":
    ...
    manifest["context_view_scope"] = {
        "included": ["plan", "side"],
        "excluded": ["front"],
        "reason": "The R20 +Y drawing contains a complete YZ side projection. The +/-X product projections are partly occluded or clipped and are not presented as a mechanically complete front context.",
    }
    ...
    manifest["project_projection_note"] = {
        "official_nominal_mm": [1300.0, 1080.0, 800.0],
        "ifc_body_local_xyz_mm": [1343.172913, 1144.75415, 797.479919],
        "interpretation": "The complete geometry-derived proxy is overlaid at the drawing's native 1:50 scale and centre-aligned; no axis is stretched to force the nominal upholstered envelope.",
