...
def render_svg(profile, view, raw_edges, proxy, official, comparison):
    width, height = 1600, 980
    left = (50, 185, 710, 700)
    right = (840, 185, 710, 700)
    left_paths = [[start, end] for start, end in raw_edges] + proxy
    left_transform = panel_transform(left_paths, *left)
    right_transform = panel_transform(official, *right)
    edge_path = svg_path([[start, end] for start, end in raw_edges], left_transform)
    proxy_path = svg_path(proxy, left_transform, close=True)
    official_path = svg_path(official, right_transform)
...
<text x="50" y="125" font-family="Arial,sans-serif" font-size="16" fill="#68798a">Left: project configured instance · Right: official 151.116.11.1 family CAD at its published default extension</text>
<text x="50" y="158" font-family="Arial,sans-serif" font-size="16" fill="#111820">Grey = actual IFC Body · Black = approved candidate · Blue = archived native DWG family reference, not force-fitted</text>
<rect x="{left[0]}" y="{left[1]}" width="{left[2]}" height="{left[3]}" rx="10" fill="#fff" stroke="#cad2d9" stroke-width="2"/>
<rect x="{right[0]}" y="{right[1]}" width="{right[2]}" height="{right[3]}" rx="10" fill="#fff" stroke="#cad2d9" stroke-width="2"/>
<text x="70" y="220" font-family="Arial,sans-serif" font-size="17" font-weight="700" fill="#111820">PROJECT CONFIGURED BODY</text>
<text x="860" y="220" font-family="Arial,sans-serif" font-size="17" font-weight="700" fill="{BLUE}">OFFICIAL NATIVE DWG · {comparison["native_dwg_code"]}</text>
...
<path class="configured-simplified-proxy geometry-derived" data-source-kind="{SOURCE_KIND}" d="{proxy_path}" fill="none" stroke="#111820" stroke-width="3" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>
<path class="official-family-native-dwg" data-source-kind="native_dwg" data-source-sha256="{comparison["source_dwg_sha256"]}" data-used-as-project-representation="false" d="{official_path}" fill="none" stroke="{BLUE}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>
...
<text x="50" y="955" font-family="Arial,sans-serif" font-size="15" fill="#68798a">Exact article fixed width matches within 0.02 mm; adjustable extension differs by design. No scaling, stretching, or blue-line substitution.</text>
</svg>'''
...
def main():
    ...
    for view, definition in VIEWS.items():
        ...
        target.write_text(render_svg(profile, view, edges, proxy, official, comparison), encoding="utf-8")
