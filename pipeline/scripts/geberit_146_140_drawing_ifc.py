#!/usr/bin/env python3
"""Approval-gated writer for exact Geberit 146.140 drawing representations.
...
from geberit_146_140_linework import ARTICLE, EXPECTED, PRODUCT_PAGE, SCOPE
...
REPRESENTATIONS = {
    "plan": ("Geberit146140Plan", "PLAN_VIEW"),
    "front": ("Geberit146140Front", "ELEVATION_VIEW"),
    "side": ("Geberit146140Side", "ELEVATION_VIEW"),
}
...
DEFAULT_MANIFEST = ROOT / "output/review/highpoly-types/geberit-146-140/manifest.json"
DEFAULT_CANDIDATE = ROOT / "output/review/highpoly-types/geberit-146-140/candidate-representations.json"
DEFAULT_LINEWORK = ROOT / "output/review/highpoly-types/geberit-146-140/official-native-dwg-linework.json"
DEFAULT_APPROVAL = ROOT / "pipeline/decisions/geberit-146-140-drawing-approval.json"
PRODUCT_PAGE_ARCHIVE = "output/review/highpoly-types/geberit-146-140/official-source/geberit-aquaclean-sela-product-page.html"
PRODUCT_PAGE_ARCHIVE_SHA256 = "8d2faaecf4e8b4e1aef578609cd21bf10fb44015cf8c916a318ebcad7bd0f157"
CURRENT_CATALOG_STATUS = "AquaClean Sela family page; current 146.220.11.1 replaces legacy 146.140.11.1"
...
def add_source_pset(model, product, product_type, approval, manifest_hash, linework_path):
    values = {
        ...
        "SourceDwgGPath": f"output/review/highpoly-types/geberit-146-140/official-source/{ARTICLE}_G.dwg",
        "SourceDwgGUrl": f"https://cdn.data.geberit.com/cad/{ARTICLE}_G.dwg",
        "SourceDwgGSha256": EXPECTED["G"],
        "SourceDwgAPath": f"output/review/highpoly-types/geberit-146-140/official-source/{ARTICLE}_A.dwg",
        "SourceDwgAUrl": f"https://cdn.data.geberit.com/cad/{ARTICLE}_A.dwg",
        "SourceDwgASha256": EXPECTED["A"],
        "SourceDwgLPath": f"output/review/highpoly-types/geberit-146-140/official-source/{ARTICLE}_L.dwg",
        "SourceDwgLUrl": f"https://cdn.data.geberit.com/cad/{ARTICLE}_L.dwg",
        "SourceDwgLSha256": EXPECTED["L"],
        "SourceDwgPPath": f"output/review/highpoly-types/geberit-146-140/official-source/{ARTICLE}_P.dwg",
        ...
        "ApprovedRepresentationIdentifiers": "Geberit146140Plan;Geberit146140Front;Geberit146140Side",
        "RepresentationSourceMapping": "Geberit146140Plan=G;Geberit146140Front=A;Geberit146140Side=L",
    ...
    pset = model.create_entity(
        ...
        Name="Pset_Geberit146140DrawingSource",
    ...
    model.create_entity(
        ...
        GlobalId=ifcopenshell.guid.new(),
        OwnerHistory=product.OwnerHistory,
        Name="Geberit 146.140 drawing source properties",
        Description=None,
...
def add_document_associations(model, product, product_type):
    documents = [
        (
            ...
            PRODUCT_PAGE,
            "Geberit official AquaClean Sela product page",
            f"{CURRENT_CATALOG_STATUS}; family and legacy article identity evidence; not drawing geometry",
        ),
    ]
    documents.extend(
        (
            f"GEBERIT-146-140-{code}-NATIVE-DWG",
            f"output/review/highpoly-types/geberit-146-140/official-source/{ARTICLE}_{code}.dwg",
    ...
    documents.append(
        (
            ...
            f"output/review/highpoly-types/geberit-146-140/official-source/{ARTICLE}_P.dwg",
...
def main():
    ...
    document_relations = [
        ...
        and (relation.RelatingDocument.Identification or "").startswith("GEBERIT-146-140-")
    ]
    ...
    source_pset = ifcopenshell.util.element.get_pset(derived_product, "Pset_Geberit146140DrawingSource") or {}
    expected_properties = {
        "SourceKind": "native_dwg",
        "ArticleNumber": ARTICLE,
        "SourceDwgGSha256": EXPECTED["G"],
    ...
    result = {
        ...
        "source_property_set": "Pset_Geberit146140DrawingSource",
