#!/usr/bin/env python3
"""Extract the configured TRAP01 IFC Body for an actual Bonsai camera review."""
...
OUTPUT = ROOT / "output/review/highpoly-types/trap01/Geberit-151.116.11.1-TRAP01-bonsai-isolated.ifc"
...
if product is None:
    raise RuntimeError("TRAP01 representative was not extracted")
...
if not any(representation.ContextOfItems.TargetView == "MODEL_VIEW" for representation in body_representations):
    raise RuntimeError("isolated TRAP01 IFC lost its actual MODEL_VIEW Body representation")
