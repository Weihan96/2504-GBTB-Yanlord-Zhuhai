#!/usr/bin/env python3
"""Load isolated Geberit Sigma01 IFC in Bonsai, add cameras, and execute Blender renders."""
...
SOURCE = PRODUCT_DIR / "Geberit-115-770-bonsai-isolated.ifc"
BLEND = PRODUCT_DIR / "Geberit-115-770-bonsai-review.blend"
MANIFEST = PRODUCT_DIR / "bonsai-review-manifest.json"
GLOBAL_ID = "2gFgcOYEXEaQWAzcKulTFt"
...
def render_cameras(obj):
    ...
    jobs = [
        ...
        # The representative's local depth axis maps to world X; the visible
        # actuator face is toward minimum world X. Preserve the official A/L
        # front/side semantics in the saved Bonsai camera session.
    ...
    for label, location, scale in jobs:
        camera_data = bpy.data.cameras.new(f"GEBERIT_115_770_CAM_{label}")
        camera = bpy.data.objects.new(f"GEBERIT_115_770_CAM_{label}", camera_data)
...
payload = {
    "schema_version": 1,
    "generator": "pipeline/scripts/render_geberit_115_770_bonsai_review.py",
