#!/usr/bin/env python3
"""Approval-gated derived IFC writer for Poliform Senzafine / WD02."""
...
PRODUCT_DIR = ROOT / "output/review/highpoly-types/wd02"
PAGE_EVIDENCE = PRODUCT_DIR / "official-source/official-product-page-evidence.json"
EXPECTED_PATH_COUNTS = {"plan": 4, "front": 3, "side": 3}

shared.REPRESENTATIVE_GLOBAL_ID = "3yuXF4PtnBHgIXDlmXFJ$7"
shared.IFC_TYPE_NAME = "WD02"
shared.PROFILE_KEY = "wd02"
shared.SCOPE = SCOPE
shared.REPRESENTATIONS = {
    "plan": ("Wd02Plan", "PLAN_VIEW"),
    "front": ("Wd02Front", "ELEVATION_VIEW"),
    "side": ("Wd02Side", "ELEVATION_VIEW"),
}
...
shared.DEFAULT_APPROVAL = ROOT / "pipeline/decisions/wd02-drawing-approval.json"
shared.PSET_NAME = "Pset_Wd02DrawingSource"
shared.DOCUMENT_ID_PREFIX = "POLIFORM-SENZAFINE-WD02-"
...
def candidate_paths(candidate: dict, access: dict) -> dict:
    if (
        ...
        or candidate.get("article_number") != "Poliform Senzafine glass wardrobe / project WD02"
        ...
        raise RuntimeError("pending WD02 candidate source gate failed")
    ...
    if (
        ...
        raise RuntimeError("WD02 official-source access record gate failed")
    paths = {}
    for view in shared.REQUIRED_VIEWS:
        ...
        if item.get("source_kind") != shared.SOURCE_KIND or item.get("official_cad_paths_mm") != []:
            raise RuntimeError(f"WD02 {view} linework must remain geometry-derived with no official CAD paths")
        paths[view] = item.get("proxy_paths_mm", [])
        if len(paths[view]) != EXPECTED_PATH_COUNTS[view]:
            raise RuntimeError(f"WD02 {view} geometry-derived path-count gate failed")
    return paths
...
def add_document_associations(model, product, product_type, access_path):
    documents = (
        ...
        (
            ...
            "Official catalogue access route; exact WD02 configuration and exact native CAD were not acquired",
        ),
        (
        ...
        (
            ...
            "WD02 manufacturer-source access record",
...
def add_source_pset(model, product, product_type, approval, manifest_hash, candidate_path, access_path):
    ...
    values = {
        ...
        "ProjectTypeCode": "WD02",
        "IFCTypeDescription": "Poliform Glass Wardrobe",
        "IFCElementType": "WARDROBE",
        "SourceProductPage": PRODUCT_PAGE,
        "SourceTechnicalCatalogRoute": TECHNICAL_CATALOG_ROUTE,
        "OfficialNativeCadStatus": "gated_resource_download_no_public_exact_WD02_asset_located_not_acquired",
        ...
        "ApprovedRepresentationIdentifiers": "Wd02Plan;Wd02Front;Wd02Side",
    ...
    model.create_entity(
        ...
        GlobalId=ifcopenshell.guid.new(),
        OwnerHistory=product.OwnerHistory,
        Name="WD02 drawing source properties",
        Description=None,
        RelatedObjects=[product, product_type],
