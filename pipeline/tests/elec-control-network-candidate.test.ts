...
test("confirmed control, network, and safety roles compile as read-only coordination zones", () => {
  ...
  expect(report.summary.hvac_control_panel_references).toBe(5);
  expect(report.summary.video_intercom_references).toBe(1);
  expect(report.summary.doorbell_references).toBe(1);
  ...
  expect(report.gates.hvac_control_field_confirmation_complete).toBe(false);
  expect(report.gates.access_control_field_confirmation_complete).toBe(false);
  ...
  expect(report.gates.five_developer_hvac_control_panels_visible).toBe(true);
  expect(report.gates.developer_hvac_control_panel_heights_visible).toBe(true);
  expect(report.gates.developer_video_intercom_visible).toBe(true);
  expect(report.gates.developer_doorbell_visible).toBe(true);
  expect(report.gates.developer_controls_remain_reference_pending_field_confirmation).toBe(true);
