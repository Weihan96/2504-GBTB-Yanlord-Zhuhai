...
test("WFIN candidate partitions every CLADDING into one confirmed finish", async () => {
  ...
  expect(report.unique_space_assignment_count).toBe(51);
  expect(report.tadelakt_count).toBe(24);
  expect(report.white_wall_count).toBe(27);
  expect(report.main_bath_dry_cladding_count).toBe(0);
